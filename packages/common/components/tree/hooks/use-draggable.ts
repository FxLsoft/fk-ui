...
import type { DropPosition, TreeNodeKey } from '../interface';

export default function useDraggable(props: { key: TreeNodeKey; refTitle: HTMLElement | undefined }) {
	...
	return {
		...
		isAllowDrop,
		dropPosition,
		setDragStatus(status: 'dragStart' | 'dragOver' | 'dragLeave' | 'dragEnd' | 'drop', e: DragEvent) {
			switch (status) {
				case 'dragStart':
