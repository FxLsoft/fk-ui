...
export default defineComponent({
	...
	setup(props) {
		...
		const renderIcon = () => {
			const type = fileType.value;
			if (type.includes('image')) {
				return <img src={props.file.url} alt={props.file.name} {...(uploadCtx?.imageLoading ? { loading: uploadCtx.imageLoading } : undefined)} />;
			}
			...
			<div class={[itemCls, `${itemCls}-${props.file.status}`]}>
				<div class={`${itemCls}-content`}>
					{uploadCtx?.listType === 'picture' && (
						<span onClick={e => onPreview(e)} class={`${itemCls}-thumbnail`}>
							{uploadCtx?.slots.image?.({ fileItem: props.file }) ?? renderIcon()}
						</span>
					)}
					<div class={`${itemCls}-name`}>
						{uploadCtx?.listType === 'text' && (
							<span class={`${itemCls}-file-icon`}>
								{uploadCtx?.slots['file-icon']?.({ fileItem: props.file }) ?? uploadCtx?.customIcon?.fileIcon?.(props.file) ?? renderFileIcon()}
							</span>
						)}
						{uploadCtx?.showLink && props.file.url ? (
							...
							>
								{uploadCtx?.slots['file-name']?.({ fileItem: props.file }) ?? uploadCtx?.customIcon?.fileName?.(props.file) ?? props.file.name}
							</a>
						) : (
						...
						{props.file.status === 'error' && (
							<Tooltip content={t('upload.error')}>
								<span class={[uploadCtx?.iconCls, `${uploadCtx?.iconCls}-error`]}>
									{uploadCtx?.slots['error-icon']?.() ?? uploadCtx?.customIcon?.errorIcon?.() ?? <IconExclamationCircleFill />}
								</span>
							</Tooltip>
						)}
					</div>
