import { cloneVNode, defineComponent, onMounted, onUnmounted, ref } from 'vue';
import ResizeObserver from 'resize-observer-polyfill';
import { getStyle } from '../_utils/style';
import { unFragment } from '../_utils/vue-utils';
import useMenuContext from './hooks/use-menu-context';
import SubMenu from './sub-menu';
import type { SubMenuProps } from './interface';
...
export default defineComponent({
	...
	setup(_, { slots }) {
		...
		function computeLastVisibleIndex() {
			...
			const childNodeList = Array.prototype.slice.call(wrapperElement.children) as HTMLElement[];
		...
		return () => {
			const renderSubMenu = (
				...
				return (
					<SubMenu
						key={`__menu-overflow-sub-menu${isMirror ? '-mirror' : ''}`}
			...
			const renderChildren = () => {
				...
				const menuItems = children.map((child, index) => {
					const item = cloneVNode(
						child,
						lastVisibleIndex.value !== null && index > lastVisibleIndex.value ? { class: overflowMenuItemClass } : { class: '' },
					);
