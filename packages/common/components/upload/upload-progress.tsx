	...
	setup(props) {
		...
		const renderIcon = () => {
			if (props.file.status === 'error') {
				return (
					<span class={[uploadCtx?.iconCls, `${uploadCtx?.iconCls}-upload`]} onClick={() => uploadCtx?.onUpload(props.file)}>
						{(uploadCtx?.showRetryButton && (uploadCtx?.slots['retry-icon']?.() ?? uploadCtx?.customIcon?.retryIcon?.())) ||
			...
			if (props.file.status === 'done') {
				return (
					<span class={[uploadCtx?.iconCls, `${uploadCtx?.iconCls}-success`]}>
						{uploadCtx?.slots['success-icon']?.() ?? uploadCtx?.customIcon?.successIcon?.() ?? <IconCheck />}
					</span>
				);
			}
			if (props.file.status === 'init') {
				return (
					<Tooltip content={t('upload.start')}>
						<span class={[uploadCtx?.iconCls, `${uploadCtx?.iconCls}-start`]} onClick={() => uploadCtx?.onUpload(props.file)}>
							{uploadCtx?.slots['start-icon']?.() ?? uploadCtx?.customIcon?.startIcon?.() ?? <IconPlayArrowFill />}
						</span>
					</Tooltip>
				);
			}
			return (
				uploadCtx?.showCancelButton && (
					<Tooltip content={t('upload.cancel')}>
						<span class={[uploadCtx?.iconCls, `${uploadCtx?.iconCls}-cancel`]} onClick={() => uploadCtx?.onAbort(props.file)}>
							{uploadCtx?.slots['cancel-icon']?.() ?? uploadCtx?.customIcon?.cancelIcon?.() ?? <IconPause />}
						</span>
					</Tooltip>
				)
			);
		...
		const renderProgress = () => {
			if (['init', 'uploading'].includes(props.file.status ?? '')) {
				...
				return <Progress type="circle" size="mini" showText={false} status={status} percent={props.file.percent} />;
			}
