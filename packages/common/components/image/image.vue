<template>
	<div :class="wrapperClassNames" :style="wrapperStyles">
		...
		<ImageFooter v-if="showFooter" :class="footerClass" :prefix-cls="prefixCls" :title="title" :description="description">
...
<script lang="ts">
import { computed, defineComponent, inject, reactive, ref, shallowRef, toRefs, watch, watchEffect } from 'vue';
...
import { DEFAULT_URL, vLazyLoad } from '../../directives/LazyLoad';
...
export default defineComponent({
	...
	setup(props, { attrs, slots, emit }) {
		const { t } = useI18n();
		const { height, width, hideFooter, title, description, src, footerPosition, defaultPreviewVisible, previewVisible, preview, previewProps } =
			toRefs(props);
		...
		const refImg = shallowRef<HTMLImageElement>();
		...
		const onLazyLoad = (el: HTMLImageElement, src: string) => {
			console.log('onLazyLoad >>', src);
			el.src = src;
		...
		watch(
			() => [src?.value, refImg.value],
			() => {
				if (isServerRendering || !refImg.value) return;
				if (props.lazy) {
					refImg.value.src = DEFAULT_URL;
				} else {
		...
		watchEffect(onCleanup => {
			const unRegister = groupContext?.registerImageUrl?.(imageId, (previewProps?.value?.src ?? src?.value) || '', preview.value);
			onCleanup(() => {
				unRegister?.();
			});
		});
