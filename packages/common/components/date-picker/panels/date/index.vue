<template>
	<div :class="classNames">
		...
		<div v-if="showTimeView" :class="`${prefixCls}-timepicker`">
			<header :class="`${prefixCls}-timepicker-title`">{{ datePickerT('datePicker.selectTime') }}</header>
			<TimePanel
				v-bind="{
					...timePickerProps,
					...disabledTimeProps,
		...
		<div v-if="showViewTabs" :class="`${prefixCls}-footer`">
			<div :class="`${prefixCls}-view-tabs`">
				<div :class="[`${prefixCls}-view-tab-pane`, { [`${prefixCls}-view-tab-pane-active`]: showDateView }]" @click="() => changeViewTo('date')">
					<IconCalendar />
					<span :class="`${prefixCls}-view-tab-pane-text`">
						{{ footerValue && footerValue.format('YYYY-MM-DD') }}
					</span>
				</div>
				<div :class="[`${prefixCls}-view-tab-pane`, { [`${prefixCls}-view-tab-pane-active`]: showTimeView }]" @click="() => changeViewTo('time')">
					<IconClockCircle />
					<span :class="`${prefixCls}-view-tab-pane-text`">
...
import type { HeaderLabelClickFunc } from '../header.vue';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		const { isRange, headerValue, footerValue, dayStartOfWeek, isSameTime, mode, showTime, currentView, disabledTime } = toRefs(props);
		...
		const disabledTimeProps = computed(() => (showTime.value && disabledTime?.value?.(getDateValue(footerValue?.value || getNow()))) || {});
