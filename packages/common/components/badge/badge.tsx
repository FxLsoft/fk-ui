...
export const COLORS = ['red', 'orangered', 'orange', 'gold', 'lime', 'green', 'cyan', 'fkblue', 'purple', 'pinkpurple', 'magenta', 'gray'] as const;
...
export default defineComponent({
	...
	setup(props, { slots }) {
		...
		const wrapperClassName = useWrapperClass(prefixCls, status?.value, slots?.default);
		...
		const computedStyleRef = computed(() => {
			...
			const computedColorStyle = !color?.value || COLORS.includes(color?.value as (typeof COLORS)[number]) ? {} : { backgroundColor: color.value };
			const mergedStyle = {
				...computedColorStyle,
				...computedDotStyle,
			} as CSSProperties;
		...
		const getDot = () => {
			...
			return (
				<span class={`${prefixCls}-number`} style={mergedStyle}>
					<span>{maxCount.value && countValue > maxCount.value ? `${maxCount.value}+` : countValue}</span>
				</span>
			);
		};
...
const useWrapperClass = (prefixCls: string, status?: BadgeStatus, children?: Slot) => {
	return computed(() => [
		prefixCls,
		{
