<template>
	<component :is="componentTag" :class="classNames" :style="{ width: siderWidth }" v-bind="resizeDirections ? { directions: resizeDirections } : {}">
		<div :class="`${prefixCls}-children`">
			<slot />
		</div>
		<div v-if="showTrigger" :class="triggerClassNames" :style="{ width: siderWidth }" @click="toggleTrigger">
...
<script lang="tsx">
import { computed, defineComponent, inject, onMounted, onUnmounted, provide, reactive, ref, toRefs } from 'vue';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const uniqueId = generateId('__layout_sider');
