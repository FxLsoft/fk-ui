...
import type { PaginationInstance } from '@erp/common';
...
const tableComponentMethodKeys: (keyof VxeTableMethods)[] = [
	'clearAll',
	'updateData',
	'loadData',
	'reloadData',
...
export default defineComponent({
	...
	setup(props, context) {
		...
		// 使用已安装的组件，如果未安装则不渲染
		const VxeUIFormComponent = VxeUI.getComponent('Form');
		const VxeUIPagerComponent = VxeUI.getComponent('Pager');
		...
		const refForm = ref() as Ref<any>;
		const refToolbar = ref() as Ref<VxeToolbarInstance>;
		const refPager = ref() as Ref<PaginationInstance>;
		...
		const computeTableProps = computed(() => {
			...
			if (proxyConfig && isEnableConf(proxyOpts)) {
				...
				if (pagerConfig && proxyOpts.seq && isEnableConf(pagerOpts)) {
					tableProps.seqConfig = Object.assign({}, seqConfig, { startIndex: (tablePage.current - 1) * tablePage.pageSize });
				}
			}
			if (editConfig) {
		...
		const initPages = () => {
			...
			const pagerOpts = computePagerOpts.value;
			const { current: current, pageSize } = pagerOpts;
			if (pagerConfig && isEnableConf(pagerOpts)) {
				if (current) {
					tablePage.current = current;
				}
				if (pageSize) {
		...
		const pageChangeEvent = params => {
			const { proxyConfig } = props;
			const { tablePage } = reactData;
			const { $event, current: current, pageSize } = params;
			const proxyOpts = computeProxyOpts.value;
			tablePage.current = current;
			tablePage.pageSize = pageSize;
		...
		const renderPager = () => {
			...
			if ((pagerConfig && isEnableConf(pagerOpts)) || slots.pager) {
				...
				if (slots.pager) {
					...
					if (pagerOpts.showSelected) {
						const selected = $xeGrid.getCheckboxRecords();
						slotVNs.push(
							h(
								'div',
								{
									class: 'vxe-grid--pager-selected',
								},
								XEUtils.toFormatString(pagerOpts.showSelected, [selected?.length]),
							),
						);
					}
		...
		const gridMethods: GridMethods = {
				...
				if (XEUtils.isString(proxyTarget)) {
					...
					const matchObj =
						toolbarConfig && isEnableConf(toolbarOpts) && buttons
							? XEUtils.findTree(buttons, item => item.code === proxyTarget, { children: 'dropdowns' })
							: null;
								...
								.then(rest => {
									...
									if (rest) {
										if (pagerConfig && isEnableConf(pagerOpts)) {
											...
											if (tablePage.current > pageCount) {
												tablePage.current = pageCount;
											}
										} else {
					...
					case 'save': {
						...
						if (ajaxMethods) {
							...
							if (editRules) {
								// 只校验新增和修改的数据
								// @ts-ignore
								restPromise = $xeTable[validConfig && validConfig.msgMode === 'full' ? 'fullValidate' : 'validate'](
			...
			setProxyInfo(options) {
				if (props.proxyConfig && options) {
					...
					if (pager) {
						if (pager.current) {
							reactData.tablePage.current = Number(pager.current);
						}
						if (pager.pageSize) {
		...
		const gridPrivateMethods: GridPrivateMethods = {
			...
			getParentHeight() {
				...
				if (el) {
					return (
						(reactData.isZMax ? getDomNode().visibleHeight : XEUtils.toNumber(getComputedStyle(el.parentNode as HTMLElement).height)) -
				...
				return gridMethods.commitProxy(params, evnt).then(rest => {
					if (code && rest && rest.status && ['query', 'reload', 'delete', 'save'].includes(code)) {
						gridMethods.dispatchEvent(
							code === 'delete' || code === 'save' ? `proxy-${code as 'delete' | 'save'}` : 'proxy-query',
							{ ...rest, isReload: code === 'reload' },
							evnt,
						);
					}
				});
