			...
			<template v-for="(cell, colIndex) in row">
				...
				<template v-else>
					<div
						...
						"
					>
						<RenderFunction v-if="dateRender" :render-func="dateRender" :date="getDateValue(cell.value)" />
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const isCellDisabled = (cellData: Cell) => !!(isFunction(disabledDate?.value) && disabledDate?.value(getDateValue(cellData.value)));
