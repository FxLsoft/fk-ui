	...
	setup(props, { slots }) {
		...
		return () => {
			...
			return (
				<div class={cls.value}>
					{(hasTitle || hasExtra) && (
						<div class={[`${prefixCls}-header`, { [`${prefixCls}-header-no-title`]: !hasTitle }]} style={props.headerStyle}>
							{hasTitle && <div class={`${prefixCls}-header-title`}>{slots.title?.() ?? props.title}</div>}
							{hasExtra && <div class={`${prefixCls}-header-extra`}>{slots.extra?.() ?? props.extra}</div>}
						</div>
					)}
