...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const getStringValue = (number: number | undefined | string) => {
			if (number === '') {
				return '';
			}
			if (!isNumber(number)) {
		...
		const nextStep = (method: StepMethods, event: Event) => {
			...
			if (isNumber(valueNumber.value)) {
				...
				nextValue = props.min === Number.NEGATIVE_INFINITY ? 0 : props.min;
			}
		...
		const handleStepButton = (event: Event, method: StepMethods, needRepeat = false) => {
			...
			// 长按时持续触发
			if (needRepeat) {
				repeatTimer = window.setTimeout(() => (event.target as HTMLElement).dispatchEvent(event), repeatTimer ? SPEED : FIRST_DELAY);
			}
		};
