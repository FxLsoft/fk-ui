import type { Key2TreeNode, Node, TreeNodeKey } from '../interface';
...
function updateParent(options: { node: Node; checkedKeySet: Set<TreeNodeKey>; indeterminateKeySet: Set<TreeNodeKey>; filterNodes?: Node[] }) {
	const { node, checkedKeySet, indeterminateKeySet, filterNodes } = options;
	let parentNode = node.parent;
	while (parentNode) {
		if (isNodeCheckable(parentNode)) {
			const parentKey = parentNode.key;
			const children =
				parentNode.children?.filter(isNodeCheckable).filter(child => {
					if (filterNodes && !filterNodes.some(v => v.key === child.key)) {
						return false;
					}
					return true;
				}) || [];
...
export function getCheckedStateByCheck(options: {
	...
	const { node, checked, checkedKeys, indeterminateKeys, checkStrictly = false, filterNodes } = options;
	...
	if (!checkStrictly) {
		...
		if (filterNodes) {
			for (let i = 0; i < childKeys.length; i++) {
				...
				if (!filterNodes.some(v => v.key === key)) {
					childKeys.splice(i, 1);
					--i;
				}
			}
			...
			for (const key of indeterminateKeySet) {
				if (!filterNodes.some(v => v.key === key)) {
					indeterminateKeySet.delete(key);
				}
			}
		...
		// 逐级更新父节点的选中状态
		updateParent({ node, checkedKeySet, indeterminateKeySet, filterNodes });
	}
