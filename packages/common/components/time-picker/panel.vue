<template>
	...
	<div v-if="!hideFooter" :class="`${prefixCls}-footer-btn-wrapper`">
		...
		<Button type="primary" size="mini" :disabled="confirmBtnDisabled || !selectedValue" @click="onConfirm">
			{{ t('datePicker.ok') }}
		</Button>
	</div>
</template>
...
export default defineComponent({
	...
	setup(props: PanelProps, { emit }) {
		const { value, visible, format, step, use12Hours, hideDisabledOptions, disabledHours, disabledMinutes, disabledSeconds, disabled } = toRefs(props);
