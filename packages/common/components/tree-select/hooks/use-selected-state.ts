...
import type { FallbackOption, LabelValue, TreeSelectValue } from '../interface';
import type { Key2TreeNode, TreeFieldNames, TreeNodeData, TreeNodeKey } from '../../tree/interface';
...
export default function useSelectedState(props: {
	...
	function getLabelValues(value: (TreeNodeKey | LabelValue)[], originValue?: LabelValue[]) {
		...
		if (validValue.length) {
			...
			validValue.forEach(item => {
				...
				if (!node) {
					const fallbackResult = isFunction(fallbackOption?.value) ? fallbackOption?.value(key) : fallbackOption?.value;
				...
				res.push({
					...
					...(originValueItem || {}),
					value: key,
					label: getLabel(item) ?? node?.title ?? originValueItem?.label ?? fallbackNodeData?.[nodeDataTitle] ?? key,
				});
			});
		}
	...
	watchEffect(() => {
		...
		computedModelValue.value = isControlled ? getLabelValues(modelValueKeys, getLabelValues(normalizeModelValue)) : undefined;
