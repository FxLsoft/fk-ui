...
hooks.add('tableKeyboardModule', {
	setupTable($xeTable) {
		...
		const handleCheckboxRangeEvent = (evnt: any, params: any) => {
			...
			const hasCheckbox = $xeTable.getColumns().some(v => v.type === 'checkbox');
			if (hasCheckbox) {
				...
				const triggerEvent = (type: 'change' | 'start' | 'end', evnt: MouseEvent) => {
					$xeTable.dispatchEvent(
						`checkbox-range-${type}` as 'checkbox-range-change' | 'checkbox-range-start' | 'checkbox-range-end',
						{ records: $xeTable.getCheckboxRecords(), reserves: $xeTable.getCheckboxReserveRecords() },
						evnt,
					);
				};
