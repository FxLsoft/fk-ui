<template>
	<Trigger
		...
		@popup-visible-change="onPanelVisibleChange"
	>
		<slot>
			<DateRangeInput
				ref="refInput"
...
<script lang="ts">
import { computed, defineComponent, inject, nextTick, onUnmounted, reactive, ref, toRefs, watch, watchEffect } from 'vue';
import { getPrefixCls } from '../_utils/global-config';
import { isArray, isBoolean } from '../_utils/is';
import pick from '../_utils/pick';
import { getFormattedValue, isValidInputValue } from '../time-picker/utils';
import { dayjs, getDateValue, getNow, getSortedDayjsArray, initializeDateLocale, isValueChange } from '../_utils/date';
...
import type { CalendarValue, DisabledTimeProps, ShortcutType, WeekStart } from './interface';
...
export default defineComponent({
	...
	emits: {
		 ...
		 */
		change: (value: (CalendarValue | undefined)[] | undefined, date: (Date | undefined)[] | undefined, dateString: (string | undefined)[] | undefined) =>
			true,
	...
	setup(props, { emit, slots }) {
		...
		// 面板显示的值
		const panelValue = computed(() => previewValue.value ?? processValue.value ?? selectedValue.value);
		...
		const { startHeaderValue, endHeaderValue, startHeaderOperations, endHeaderOperations, resetHeaderValue, setHeaderValue } = useRangeHeaderValue(
			reactive({
				mode,
				startHeaderMode,
				endHeaderMode,
		...
		function getSortedDayjsArrayByExchangeTimeOrNot(newValue: Dayjs[]) {
			...
			if (hasTime.value && !mergedExchangeTime.value) {
				sortedValue = [getMergedOpValue(sortedValue[0], newValue[0]), getMergedOpValue(sortedValue[1], newValue[1])];
			}
			return sortedValue;
		}

		function confirm(value: Array<Dayjs | undefined> | undefined, showPanel?: boolean, emitOk?: boolean) {
		...
		function onInputChange(e: any) {
			...
			const formattedPanelValue = getFormattedValue(panelValue.value, computedFormat.value) as Array<string | undefined>;
