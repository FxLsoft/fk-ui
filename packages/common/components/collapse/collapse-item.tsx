import { Transition, computed, defineComponent, getCurrentInstance, inject, ref, watch } from 'vue';
...
export default defineComponent({
	...
	setup(props, { slots }) {
		...
		const key = instance && isNumber(instance?.vnode.key) ? instance.vnode.key : String(instance?.vnode.key ?? '');
		...
		return () => {
			return (
				<div class={cls.value}>
					<div role="button" aria-disabled={props.disabled} aria-expanded={isActive.value} tabindex="0" class={headerCls.value} onClick={handleClick}>
						{expandIconRender()}
