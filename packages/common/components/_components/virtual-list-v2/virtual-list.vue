		...
		>
			...
			>
				<VirtualListItem
					...
					:key="item[isFunction(itemKey) ? itemKey(item) : itemKey] ?? start + index"
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const style = computed(() => {
			return {
				maxHeight: isNumber(height.value) ? `${height.value}px` : height.value,
				overflow: 'auto',
			};
		});
		...
		const dataKeys = computed(() =>
			data.value.map((item: any, index) => {
				return (item[isFunction(itemKey.value) ? itemKey.value(item) : itemKey.value] ?? index) as string | number;
			}),
		);
