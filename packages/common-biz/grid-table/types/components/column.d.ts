import type { VxeTooltipPropTypes } from '../core/custom';
...
import type { VxeTableConstructor, VxeTableDefines, VxeTablePropTypes } from './table';
...
export namespace VxeColumnPropTypes {
	...
	export type Formatter<D = any> = ((params: { cellValue: any; column: VxeTableDefines.ColumnInfo<D>; row: D }) => string | number) | any[] | string;

	export type FooterFormatter<D = any> =
		| ((params: { itemValue: any; column: VxeTableDefines.ColumnInfo<D>; row: any; items: any[]; _columnIndex: number }) => string | number)
		| any[]
		| string;

	export type Sortable = boolean;
	export type SortBy<D = any> = string | ((params: { row: D; column: VxeTableDefines.ColumnInfo<D> }) => string | number);
	...
	export interface FilterRender {
		 ...
		 * 筛选比较操作符
		 */
		compareOperator?: 'between' | 'in' | 'like' | 'eq' | 'include';
		/**
		 * 筛选比较字段
		 */
		compareKey?: string;
