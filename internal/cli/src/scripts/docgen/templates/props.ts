...
const tmpl = (props: PropDescriptor[], { isInterface = false }: { isInterface: boolean }, lang: string) => {
	const displayableProps = props.filter(({ name, description, tags }) => description || ['disabled'].includes(name) || lang in (tags ?? {}));
	...
	const content = displayableProps
		.map(prop => {
			...
			const getName = () =>
				`${isInterface ? name : toKebabCase(name)}${tags?.vModel || name === 'modelValue' ? ' **(v-model)**' : ''}${
			...
			const getDefaultValue = () => {
				if (tags?.defaultValue) {
					// @ts-ignore-next-line
					return trimStr(tags.defaultValue[0]?.description || '');
				}
				...
				if (
					(fixedType.includes('[]') || fixedType.includes('Array') || fixedType.includes('array')) &&
					defaultValue?.func &&
				...
				return trimStr(defaultValue?.value || '-');
			};
			const defaultValueStr = trimStr(getDefaultValue() || '').replace(/[\r|\n|\s]+/g, ' ');
			let lineContent = `|${getName()}|${getDescription()}|\`${getType()}\`|\`${defaultValueStr}\`|`;
