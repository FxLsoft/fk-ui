...
import XEUtils, { clone, get, isEqual, set } from 'xe-utils';
...
const tableEditMethodKeys: (keyof TableEditMethods)[] = [
	'insert',
	'insertAt',
	'insertNextAt',
	'remove',
...
hooks.add('tableEditModule', {
	setupTable($xeTable) {
		...
		function insertTreeRow(newRecords: any[], isAppend: boolean) {
			...
			newRecords.forEach(item => {
				...
				const matchObj = parentRowId
					? XEUtils.findTree(tableFullTreeData, item => parentRowId === item[rowField], { children: mapChildrenField })
					: null;
				if (matchObj) {
		...
		const handleInsertRowAt = (records: any, row: any, isInsertNextRow?: boolean) => {
			...
			const newRecords: any[] = reactive(
				$xeTable.defineField(
					records.map((record: any) => Object.assign(treeConfig && transform ? { [mapChildrenField]: [], [childrenField]: [] } : {}, record)),
				),
			);
		...
		const innerClearEdit = (evnt: Event, isForce = false, isRecover = true) => {
			const { editStore } = reactData;
			const { actived, focused } = editStore;
			const { row, column, $copyRow } = actived;
			const { trigger } = computeEditOpts.value;
			// 需要手动清除激活状态
			if (trigger === 'manual' && !isForce && !isEqual(row, $copyRow)) {
				return;
			}
			if (row || column) {
				syncActivedCell();
				...
				if (isRecover === true) {
					Object.keys($copyRow).forEach(key => {
						set(row, key, get($copyRow, key));
					});
				}
				actived.$copyRow = null;
				$xeTable.updateFooter();
		...
		editMethods = {
			...
			getUpdateRecords() {
				...
				if (keepSource) {
					syncActivedCell();
					if (treeConfig) {
						return XEUtils.filterTree(tableFullData, row => $xeTable.isUpdateByRow(row), treeOpts as any);
					}
			 ...
			 * @returns
			 */
			forceClearEdit(isRecover = false) {
				innerClearEdit(null, true, isRecover);
				return nextTick();
			},
			/**
			 * 清除所选中源状态
		...
		editPrivateMethods = {
					...
					if (!$xeTable.isPendingByRow(row)) {
						if (actived.row !== row || (mode === 'cell' ? actived.column !== column : false)) {
							...
							if (!beforeEditMethod || beforeEditMethod({ ...params, $table: $xeTable, $grid: $xeTable.xegrid })) {
								...
								if (trigger === 'manual') {
									actived.$copyRow = clone(row, true);
								}
			...
			handleFocus(params) {
				...
				if (isEnableConf(editRender)) {
					...
					if (!autoFocus && compRender) {
						autoFocus = compRender.tableAutoFocus;
					}
					if (!autoSelect && compRender) {
						autoSelect = compRender.tableAutoSelect;
					}
					// 如果指定了聚焦 class
			...
			handleSelected(params, evnt) {
				...
				const { row, column } = params;
				const isMouseSelected = mouseConfig && mouseOpts.selected && column.canMouseSelected !== false;
				const selectMethod = () => {
