<template>
	<div :class="classNames">
		...
		<div :class="`${prefixCls}-panel-wrapper`">
			...
			<template v-else>
				<WeekPanel v-if="mode === 'week'" v-bind="commonPanelProps" :day-start-of-week="dayStartOfWeek" />
				<MonthPanel v-else-if="mode === 'month'" :abbreviation="abbreviation" v-bind="commonPanelProps" />
				...
				<DatePanel
					...
					@time-picker-select="onTimePickerSelect"
				/>
				<PanelFooter
					...
					@today-btn-click="onTodayBtnClick"
					@confirm-btn-click="onConfirmBtnClick"
				>
...
import type { CalendarValue, DisabledDate, DisabledTime, HeaderIcons, HeaderOperations, ShortcutType, WeekStart } from './interface';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		function getShortcutValue(shortcut: ShortcutType) {
			...
			return getDayjsValue((isFunction(value) ? value() : value) as CalendarValue, shortcut.format || format.value);
		}
