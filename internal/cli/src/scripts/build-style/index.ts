...
const run = async ({ material }: { material: boolean }) => {
	...
	fs.writeFileSync(paths.resolvePath(material ? 'dist/index.less' : 'dist/index.less'), "@import '../es/index.less';\n\n");

	fs.writeFileSync(paths.resolvePath(material ? 'dist/index.css' : 'dist/index.css'), result.css);

	const compress = new CleanCSS().minify(result.css);

	fs.writeFileSync(paths.resolvePath(material ? 'dist/index.min.css' : 'dist/index.min.css'), compress.styles);
