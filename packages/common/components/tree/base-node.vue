		...
		>
			...
			<span :class="`${prefixCls}-title-text`">
				...
				<span v-if="draggable" :class="[`${prefixCls}-icon`, `${prefixCls}-drag-icon`]">
					...
					<RenderFunction v-else-if="treeDragIcon" :render-func="treeDragIcon" :node="treeNodeData" v-bind="nodeStatus" />
...
<script lang="ts">
import { computed, defineComponent, reactive, ref, shallowRef, toRefs } from 'vue';
	...
	setup(props) {
		...
		const classNames = computed(() => [
			`${prefixCls}`,
			{
				...
				[`${prefixCls}-disabled-selectable`]: !selectable.value && !treeContext.treeProps?.disableSelectActionOnly,
				[`${prefixCls}-disabled`]: disabled.value,
			},
		]);

		const refTitle = shallowRef<HTMLElement>();
		...
		const treeTitle = computed(() => (treeContext.nodeTitle ? () => treeContext.nodeTitle?.(treeNodeData.value, nodeStatus, node.value) : undefined));
		const extra = computed(() => (treeContext.nodeExtra ? () => treeContext.nodeExtra?.(treeNodeData.value, nodeStatus, node.value) : undefined));
