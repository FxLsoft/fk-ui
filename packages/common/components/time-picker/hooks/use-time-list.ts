...
import type { PanelProps, TimeList } from '../interface';
import type { ComputedRef } from 'vue';

interface TimeListProps
	extends Pick<PanelProps, 'format' | 'step' | 'use12Hours' | 'disabledHours' | 'disabledMinutes' | 'disabledSeconds' | 'hideDisabledOptions'> {
...
export default function useTimeList(props: TimeListProps): {
	...
	const hours = computed(() => {
		...
		if (hideDisabledOptions.value && disabledList.length) {
			list = list.filter(h => !disabledList.includes(h));
		}
		return list.map(h => ({
	...
	const minutes = computed(() => {
		...
		if (hideDisabledOptions.value && disabledList.length) {
			list = list.filter(m => !disabledList.includes(m));
		}
		return list.map(m => ({
	...
	const seconds = computed(() => {
		...
		if (hideDisabledOptions.value && disabledList.length) {
			list = list.filter(s => !disabledList.includes(s));
		}
		return list.map(s => ({
