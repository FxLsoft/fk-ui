			...
			<div :class="`${prefixCls}-range`">
				<div :class="`${prefixCls}-range-wrapper`">
					<template v-if="startHeaderMode || endHeaderMode">
						...
						<MonthPanel v-else-if="startHeaderMode === 'month'" v-bind="startPanelProps" :abbreviation="abbreviation" />
						<MonthPanel v-else-if="endHeaderMode === 'month'" v-bind="endPanelProps" :abbreviation="abbreviation" />
					</template>
					<template v-else>
						<!-- week -->
...
import type { CalendarValue, Mode, RangeDisabledDate, RangeDisabledTime, RangePickerProps, ShortcutType, StartHeaderProps, WeekStart } from './interface';
	...
	setup(props, { emit }) {
		...
		function getShortcutValue(shortcut: ShortcutType) {
			return getDayjsValue(
				normalizeRangeValue(isFunction(shortcut.value) ? shortcut.value() : shortcut.value) as CalendarValue[],
		...
		function getDisabledDateFunc(index: 0 | 1) {
			return isFunction(disabledDate?.value) ? (current: Date) => disabledDate?.value?.(current, index === 0 ? 'start' : 'end') || false : undefined;
		}

		function getDisabledTimeFunc(index: 0 | 1) {
			return isFunction(disabledTime?.value) ? (current: Date) => disabledTime?.value?.(current, index === 0 ? 'start' : 'end') || false : undefined;
		}
