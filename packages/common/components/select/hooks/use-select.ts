...
import type { VirtualListRef } from '../../_components/virtual-list-v2/interface';
import type { FilterOption, SelectFieldNames, SelectOption } from '../interface';
...
export const useSelect = ({
	...
	const { validOptions, optionInfoMap, validOptionInfos, enabledOptionKeys, getNextSlotOptionIndex, addSlotOptionInfo, removeSlotOptionInfo } = useOptions({
		options,
		extraOptions,
		inputValue,
		filterOption,
	...
	watch(popupVisible, visible => {
		if (visible) {
			...
			let _activeKey = defaultActiveFirstOption?.value ?? true ? enabledOptionKeys.value[0] : undefined;
