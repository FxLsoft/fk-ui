...
import type { CheckableType, Node, SelectableType, TreeFieldNames, TreeNodeData, TreeNodeKey } from '../interface';
...
export function generateTreeData(treeData: TreeNodeData[], treeProps: TreeProps) {
	function preOrder(tree: TreeNodeData[] | undefined, parentNode?: Node) {
		...
		tree.forEach((treeNodeData, index) => {
			...
			node.children = preOrder(treeNodeData[(fieldNames?.children || 'children') as keyof TreeNodeData] as TreeNodeData['children'], node);
			nodes.push(node);
		});
 ...
 * @param key 节点 key
 */
export function getNodeByKey(nodes: Node[], key: TreeNodeKey) {
	for (let i = 0; i < nodes.length; i += 1) {
		const node = nodes[i];
		if (node.key === key) return node;

		if (node.children) {
			const targetNode = getNodeByKey(node.children, key);
