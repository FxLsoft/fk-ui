...
export default defineComponent({
	...
	emits: {
		 ...
		 * @returns
		 */
		pageChange: (page: { current: number; pageSize: number }) => true,
	},
	/**
	 * @zh 分页按钮
	 * @en Page item
	...
	setup(props, { emit, slots }) {
		...
		const handleClick = (page: number) => {
			...
			if (page !== computedCurrent.value && isNumber(page) && !props.disabled) {
				...
				emit('change', page);
				emit('pageChange', { pageSize: _pageSize.value, current: _current.value });
			}
		};

		const handlePageSizeChange = (pageSize: number) => {
			if (props.autoAdjust) {
				const index = _pageSize.value * (computedCurrent.value - 1) + 1;
			...
			emit('pageChange', { pageSize, current: _current.value });
		};
		...
		const getPageItemElement = (type: PageItemType, props: Data = {}) => {
			if (type === 'more') {
				return <EllipsisPager v-slots={{ default: slots['page-item-ellipsis'] }} {...props} {...pagerProps} />;
			}
			if (type === 'previous') {
				return <StepPager v-slots={{ default: slots['page-item-step'] }} type="previous" {...props} {...pagerProps} />;
			}
			if (type === 'next') {
				return <StepPager v-slots={{ default: slots['page-item-step'] }} type="next" {...props} {...pagerProps} />;
			}
		...
		return () => {
			...
			return (
				<div class={cls.value}>
					{props.showTotal && <span class={`${prefixCls}-total`}>{slots.total?.({ total: props.total }) ?? t('pagination.total', props.total)}</span>}
					{props.showPageSize && (
						<PageOptions
							disabled={props.disabled}
