<template>
	<div :class="classNames" v-bind="$attrs" :style="computedStyle">
		...
		<div v-if="computedHasCollapseButton" :class="`${computedPrefixCls}-collapse-button`" @click="onCollapseBtnClick">
...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const computedHasCollapseButton = computed(() => !['horizontal', 'popButton'].includes(mode.value) && !inTrigger.value && showCollapseButton.value);
		...
		const computedStyle = computed(() => {
			const pxCollapsedWidth = isNumber(collapsedWidth.value) ? `${collapsedWidth.value}px` : undefined;
