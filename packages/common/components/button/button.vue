<template>
	<template v-if="href">
		...
		>
			<span v-if="loading || $slots.icon" :class="`${prefixCls}-icon`">
				<IconLoading v-if="loading" :spin="true" />
	...
	<template v-else>
		<button :class="[cls, { [`${prefixCls}-only-icon`]: $slots.icon && !$slots.default }]" :type="htmlType" :disabled="mergedDisabled" @click="handleClick">
			<span v-if="loading || $slots.icon" :class="`${prefixCls}-icon`">
				<IconLoading v-if="loading" :spin="true" />
				<slot v-else name="icon" />
			</span>
...
export default defineComponent({
	...
	props: {
		/**
		 * @zh 按钮的类型，分为五种：次要按钮、主要按钮、Info按钮、虚框按钮、线性按钮、文字按钮。
		 * @en Button types are divided into five types: secondary, primary, info, dashed, outline and text.
		 ...
		 */
		htmlType: {
			type: String as PropType<Pick<ButtonHTMLAttributes, 'type'>['type']>,
			default: 'button',
		},
		/**
