...
import type { FieldData, FieldRule, ValidatedError } from './interface';
import type { Size } from '../_utils/constant';
import type { FormItemInfo } from './context';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const { id, model, layout, disabled, labelAlign, labelColProps, wrapperColProps, labelColStyle, wrapperColStyle, size, rules } = toRefs(props);
		...
		const scrollToField = (field: string, options?: ScrollIntoViewOptions) => {
			...
			scrollToElement(fieldNode);
		};

		const scrollToElement = (node: Element, options?: ScrollIntoViewOptions) => {
			if (node) {
		...
		const validate = (callback?: (errors: undefined | Record<string, ValidatedError>) => void): Promise<undefined | Record<string, ValidatedError>> => {
		...
		const validateField = (field: string | string[], callback?: (errors: undefined | Record<string, ValidatedError>) => void) => {
			const list: Promise<any>[] = [];
		...
		return {
			...
			innerScrollToElement: scrollToElement,
		};
	},
	methods: {
		/**
		 ...
		 * @param options
		 */
		scrollToElement(node: Element, options?: ScrollIntoViewOptions) {
			return this.innerScrollToElement(node, options);
		},
	},
});
</script>
