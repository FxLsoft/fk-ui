<template>
	<div :class="classNames">
		...
		<div :class="`${prefixCls}-suffix`">
			<IconHover v-if="allowClear && !mergedDisabled && displayValue" :prefix="prefixCls" :class="`${prefixCls}-clear-icon`" @click="onClear">
				<IconClose />
			</IconHover>
...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const { mergedSize: _mergedSize, mergedDisabled, mergedError, feedback } = useFormItem({ size, disabled, error });
