...
export default function useMenuOpenState(props: MenuOpenStateProps) {
	const { modelValue, defaultValue, autoOpen, autoOpenSelected, subMenuKeys, selectedKeys, menuData, accordion } = toRefs(props);
	...
	onMounted(() => {
		...
		if (autoOpenSelected.value) {
			...
			if (pathList.length && (!autoOpen.value || accordion.value)) {
				newOpenKeys = accordion.value ? pathList[0] : [...new Set(([] as string[]).concat(...pathList))];
			}
		}
