<template>
	<div :class="cls" v-bind="eventHandlers">
		<span v-if="type === 'slider'" :style="sliderStyle" :class="[`${prefixCls}-item`, `${prefixCls}-item-active`]" />
		<template v-else>
			<span
	...
	setup(props, { emit }) {
		...
		const onClick = (event: MouseEvent) => {
			...
			if (props.type === 'slider') {
				...
				const index = Number.parseInt((event.target as HTMLElement).dataset.index ?? '', 10);
		...
		const cls = computed(() => [`${prefixCls}`, `${prefixCls}-${props.type}`, `${prefixCls}-${props.position}`]);
