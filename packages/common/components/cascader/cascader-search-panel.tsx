	...
	setup(props, { slots }) {
		...
		const renderContent = () => {
			...
			if (props.options.length === 0) {
				return <div class={`${prefixCls}-list-empty`}>{slots.empty?.() ?? configCtx?.slots.empty?.({ component: 'cascader' }) ?? <Empty />}</div>;
			}
			return (
				<ul
					role="menu"
		...
		return () => <Scrollbar class={[`${prefixCls}-panel`, `${prefixCls}-search-panel`]}>{renderContent()}</Scrollbar>;
	},
});
