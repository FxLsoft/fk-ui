...
<script lang="ts">
import { computed, defineComponent, onUnmounted, reactive, ref, toRefs, watch, watchEffect } from 'vue';
import { dayjs, getDateValue, getNow, initializeDateLocale, isValueChange } from '../_utils/date';
...
import type { CalendarValue, DisabledTimeProps, FormatFunc, ShortcutType, WeekStart } from './interface';
...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const { format: computedFormat, valueFormat: returnValueFormat, parseValueFormat } = useFormat(reactive({ format, mode, showTime, valueFormat }));

		const inputFormat = computed(() =>
			format && isFunction(format.value) ? (value: Dayjs) => (format.value as FormatFunc)?.(getDateValue(value)) : computedFormat.value,
		);
