...
import type { RenderContent } from '../_utils/types';
...
export interface DrawerConfig {
	 ...
	 */
	onBeforeCancel?: (e: Event) => boolean;
	/**
	 * @zh 抽屉打开后（动画结束）触发
	...
	/**
	 * @zh 抽屉关闭前触发，有clickOutside触发
	 * @en Triggered before drawer is closed
	 * @version 1.0.0
	 */
	onBeforeOutsideClose?: (e?: Event) => boolean;
}

export type DrawerUpdateConfig = Omit<
	DrawerConfig,
	'title' | 'content' | 'onOk' | 'onCancel' | 'onBeforeOk' | 'onBeforeCancel' | 'onOpen' | 'onClose' | 'onBeforeOpen' | 'onBeforeClose' | 'header' | 'footer'
>;
