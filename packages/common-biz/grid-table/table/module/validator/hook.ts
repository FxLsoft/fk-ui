	...
	setupTable($xeTable) {
					...
					if (isFull || !validRuleErr) {
						...
						columns.forEach((column: any) => {
							if ((isFull || !validRuleErr) && XEUtils.has(editRules, column.property)) {
								colVailds.push(
									validatorPrivateMethods.validCellRules('all', row, column).catch(({ rule, rules }: any) => {
										const rest = {
											rule,
											rules,
		...
		validatorMethods = {
			...
			clearValidate(rows, fieldOrColumn) {
				...
				const colList = (
					XEUtils.isArray(fieldOrColumn) ? fieldOrColumn : (fieldOrColumn ? [fieldOrColumn] : []).map(column => handleFieldOrColumn($xeTable, column))
				) as VxeTableDefines.ColumnInfo<any>[];
				let validErrMaps: Record<
					string,
					{
						row: any;
						column: any;
		...
		validatorPrivateMethods = {
							...
							if (validType === 'all' || !trigger || validType === trigger) {
								if (validator) {
									...
									if (XEUtils.isString(validator)) {
										...
										if (gvItem) {
											const tcvMethod = gvItem.tableCellValidatorMethod;
											if (tcvMethod) {
												...
												customValid.catch((e: any) => {
													...
													errorRules.push(
														new Rule({
															type: 'custom',
															trigger,
															content: e && e.message ? e.message : rule.content,
															rule: new Rule(rule),
														}),
													);
												}),
			...
			showValidTooltip(params) {
				...
				$xeTable.dispatchEvent('valid-error', params, null);
				if (validTip) {
					if (validTip && !height && tableData.length < 2) {
