...
function renderTitlePrefixIcon(params: VxeTableDefines.CellRenderHeaderParams) {
	...
	const titlePrefix = column.titlePrefix;
	return titlePrefix
		? [
				h('i', {
...
function getFooterContent(params: VxeTableDefines.CellRenderFooterParams) {
	...
	if (renderOpts) {
		...
		if (compConf) {
			const rtFooter = compConf.renderTableFooter;
			if (rtFooter) {
...
export const Cell = {
	createColumn($xeTable: VxeTableConstructor & VxeTablePrivateMethods, columnOpts: VxeTableDefines.ColumnOptions | VxeTableDefines.ColumnInfo) {
		...
		const { computeEditOpts, computeCheckboxOpts, computeFilterOpts } = $xeTable.getComputeMaps();
		const checkboxOpts = computeCheckboxOpts.value;
		const editOpts = computeEditOpts.value;
		const hasDefaultFilter = !computeFilterOpts.value.mode && filters;
		const renConfs: any = {
				...
				if (editConfig && editRender) {
					...
					renConfs.renderCell =
						editOpts.mode === 'cell'
							...
							: Cell.renderRowEdit;
				} else if (hasDefaultFilter && sortable) {
					renConfs.renderHeader = Cell.renderSortAndFilterHeader;
				} else if (sortable) {
					renConfs.renderHeader = Cell.renderSortHeader;
				} else if (hasDefaultFilter) {
	...
	renderHeaderTitle(params: VxeTableDefines.CellRenderHeaderParams) {
		...
		if (renderOpts) {
			const compConf = renderer.get(renderOpts.name);
			if (compConf) {
				const rtHeader = compConf.renderTableHeader;
				if (rtHeader) {
	...
	renderDefaultCell(params: VxeTableDefines.CellRenderBodyParams) {
		...
		if (renderOpts) {
			const compConf = renderer.get(renderOpts.name);
			if (compConf) {
				const rtCell = compConf.renderTableCell;
				const rtDefault = compConf.renderTableDefault;
	...
	renderTreeIcon(params: VxeTableDefines.CellRenderBodyParams, cellVNodes: VxeComponentSlotType[]) {
		...
		if (!isHidden) {
			...
			isActivated = !!treeExpandedMaps[rowid];
			if (lazy) {
									...
									[
										h('i', {
											class: [
												'vxe-tree--node-btn',
												isLazyLoading
													? iconLoaded || getIcon().TABLE_TREE_LOADED
													: isActivated
													? iconOpen || getIcon().TABLE_TREE_OPEN
													: iconClose || getIcon().TABLE_TREE_CLOSE,
											],
										}),
									],
								),
	...
	renderExpandCell(params: VxeTableDefines.CellRenderBodyParams) {
		...
		if (!isHidden) {
			const rowid = getRowid($table, row);
			isActivated = !!rowExpandedMaps[rowid];
			if (lazy) {
						...
						[
							h('i', {
								class: [
									...
									isLazyLoading
										? iconLoaded || getIcon().TABLE_EXPAND_LOADED
										: isActivated
										? iconOpen || getIcon().TABLE_EXPAND_OPEN
										: iconClose || getIcon().TABLE_EXPAND_CLOSE,
								],
							}),
						],
				  )
	...
	renderExpandData(params: VxeTableDefines.CellRenderDataParams) {
		...
		if (contentRender) {
			...
			if (compConf) {
				const rtExpand = compConf.renderTableExpand;
				if (rtExpand) {
				...
				h(
					...
					[
						h('i', {
							class: [
								'vxe-sort--asc-btn',
								iconAsc || getIcon().TABLE_SORT_ASC,
								{
						...
						h('i', {
							class: [
								'vxe-sort--desc-btn',
								iconDesc || getIcon().TABLE_SORT_DESC,
								{
	...
	renderEditHeader(params: VxeTableDefines.CellRenderHeaderParams) {
		const { $table, column } = params;
		const { props } = $table;
		const { computeEditOpts, computeFilterOpts } = $table.getComputeMaps();
		...
		return (
			...
			.concat(hasDefaultFilter ? Cell.renderFilterIcon(params) : []);
	},
	// 行格编辑模式
	...
	runRenderer(params: VxeTableDefines.CellRenderBodyParams, isEdit: boolean) {
		...
		const rtEdit = compConf ? compConf.renderTableEdit : null;
