...
export default defineComponent({
	...
	setup(props) {
		...
		const isVMScrollProcess = () => {
			...
			return !!(_isResize || (lastScrollTime && Date.now() < lastScrollTime));
		};
		...
		const renderLine = (params: any) => {
			...
			if (treeConfig && treeNode && treeOpts.showLine) {
				return [
					h(
						'div',
						{
		...
		const renderColumn = (
			...
			const compCellClassName = compConf ? compConf.tableCellClassName : null;
			const compCellStyle = compConf ? compConf.tableCellStyle : '';
			...
			const showValidTip = editRules && validOpts.showMessage && (height || tableData.length > 1);
			...
			if (fixedHiddenColumn && (allColumnOverflow ? isAllOverflow : allColumnOverflow)) {
				tdVNs.push(
					h('div', {
						...
						style: {
							maxHeight: hasEllipsis && rowHeight ? `${rowHeight}px` : '',
						},
					}),
				);
			} else {
			...
			return h(
				'td',
				{
					...
					key: columnOpts.useKey ? colid : $columnIndex,
					...attrs,
					style: Object.assign(
						{
							height: hasEllipsis && rowHeight ? `${rowHeight}px` : '',
						},
						XEUtils.isFunction(compCellStyle) ? compCellStyle(params) : compCellStyle,
						XEUtils.isFunction(cellStyle) ? cellStyle(params) : cellStyle,
					),
					...tdOns,
				},
				tdVNs,
		...
		const renderRows = (fixedType: any, tableData: any, tableColumn: any) => {
			const { stripe, rowClassName, rowStyle, showOverflow: allColumnOverflow, editConfig, treeConfig } = tableProps;
			const { hasFixedColumn, treeExpandedMaps, scrollYLoad, rowExpandedMaps, expandColumn, selectRadioRow, pendingRowMaps, pendingRowList } =
				tableReactData;
			const { fullAllDataRowIdData } = tableInternalData;
			const checkboxOpts = computeCheckboxOpts.value;
				...
				rows.push(
					h(
						'tr',
						{
							...
							key: rowOpts.useKey || treeConfig ? rowid : $rowIndex,
							...trOn,
						},
		...
		const syncBodyScroll = (
			...
			scrollTop: number,
			elem1: XEBodyScrollElement | null,
			elem2: XEBodyScrollElement | null,
		) => {
			if (elem1 || elem2) {
				if (elem1) {
