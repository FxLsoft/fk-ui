...
import XEUtils, { get } from 'xe-utils';
import {
	addClass,
	browse,
	getEventTargetNode,
...
import type { TooltipInstance } from '@erp/common';
...
export default defineComponent({
	...
	setup(props, context) {
		...
		// 使用已安装的组件，如果未安装则不渲染
		const VxeUILoadingComponent = VxeUI.getComponent('Loading');
		...
		const refTooltip = ref() as Ref<TooltipInstance>;
		const refCommTooltip = ref() as Ref<TooltipInstance>;
		const refValidTooltip = ref() as Ref<TooltipInstance>;
					...
					if ((rowList ? row : XEUtils.isNumber(row)) && col && (rowspan || colspan)) {
						...
						if (rowspan > 1 || colspan > 1) {
							const mcIndex = XEUtils.findIndexOf(
								mList,
								item =>
									(item._row === row || getRowid($xeTable, item._row) === getRowid($xeTable, row)) &&
									((item as any)._col.id === col || item._col.id === (col as VxeTableDefines.ColumnInfo).id),
							);
		...
		const restoreCustomStorage = () => {
			...
			if (customConfig && (isCustomResizable || isCustomVisible || isCustomFixed || isCustomSort)) {
				...
				if (restoreStore) {
					return Promise.resolve(restoreStore({ id: tableId, type: 'restore', storeData }))
						.then(storeData => {
							if (!storeData) {
								return;
							}
		...
		const updateAfterFullData = () => {
			...
			if (!allRemoteFilter || !allRemoteSort) {
				...
				tableFullColumn.forEach(column => {
					...
					if (!allRemoteFilter) {
						if (!filterOpts.mode && filters && filters.length) {
							const valueList: any[] = [];
						...
						} else if (filterOpts.mode === 'header') {
							const value = get(filterStore.model, column.field);
							if (value !== undefined && value !== null) {
								const valueList = Array.isArray(value) ? value : [value];
								filterColumns.push({ column, valueList, itemList: [{ value, label: column.title } as VxeTableDefines.FilterOption] });
							}
						}
					}
					if (!allRemoteSort && sortable && order) {
						orderColumns.push({ column, field, order, sortTime: column.sortTime });
					}
				});
				...
				if (!allRemoteFilter && filterColumns.length) {
					const handleFilter = (row: any) => {
						return filterColumns.every(({ column, valueList, itemList }) => {
							...
							const compFilterMethod = compConf ? compConf.tableFilterMethod : null;
							const tdFilterMethod = compConf
								? filterOpts.mode === 'header'
									? compConf.tableHeaderFilterDefaultMethod
					...
					if (treeConfig && transform) {
						...
						if (allSortMethod) {
							...
							tableTree = XEUtils.orderBy(
								tableTree,
								orderColumns.map(({ column, order }) => [getOrderField(column), order]),
							);
						}
						tableData = tableTree;
					} else {
				...
				layoutList.forEach(layout => {
					...
					if (layout === 'header') {
						...
						if (fixedWrapperElem) {
							...
							fixedWrapperElem.style.height = `${
								(customHeight > 0 ? customHeight - headerHeight - footerHeight : tableHeight) +
								headerHeight +
								footerHeight -
								scrollbarHeight * (showFooter ? 2 : 1)
							}px`;
							fixedWrapperElem.style.width = `${fixedColumn.reduce(
						...
						if (fixedType) {
							...
							if (!reactData.expandColumn && (scrollYLoad || (allColumnOverflow ? isAllOverflow : allColumnOverflow))) {
								// TODO spanMethod
								if (!mergeList.length && !(keyboardConfig && keyboardOpts.isMerge)) {
									renderColumnList = fixedColumn;
								} else {
									renderColumnList = visibleColumn;
								}
					...
					if (colgroupElem) {
						XEUtils.arrayEach(colgroupElem.children, (colElem: any) => {
							...
							if (fullColumnIdData[colid]) {
								...
								if (layout === 'header') {
									cellOverflow =
										XEUtils.isUndefined(showHeaderOverflow) || XEUtils.isNull(showHeaderOverflow)
											? allColumnHeaderOverflow
											: showHeaderOverflow;
								} else if (layout === 'footer') {
									cellOverflow =
										XEUtils.isUndefined(showFooterOverflow) || XEUtils.isNull(showFooterOverflow)
		...
		const handleCheckedAllCheckboxRow = (value: boolean, isForce?: boolean) => {
			...
			const indeterminateField = checkboxOpts.indeterminateField;
		...
		const handleAsyncRowExpand = (row: any): Promise<void> => {
			return new Promise(resolve => {
				...
				if (loadMethod) {
					...
					loadMethod({ $table: $xeTable, row, rowIndex: tableMethods.getRowIndex(row), $rowIndex: tableMethods.getVMRowIndex(row) })
						.then(() => {
		...
		const computeScrollLoad = () => {
			return nextTick().then(() => {
				...
				if (scrollXLoad) {
					...
					if (sXOpts.customScroll) {
						if (sXOpts.customScroll(scrollXStore, internalData)) {
				...
				if (scrollYLoad) {
					...
					if (sYOpts.customScroll) {
						if (sYOpts.customScroll(scrollYStore, internalData)) {
		...
		const parseColumns = () => {
			...
			if (
				...
				!internalData.visibleColumn.every((column, index) => column === visibleColumn[index])
			) {
				tableMethods.clearMergeCells();
				tableMethods.clearMergeFooterItems();
			}
		...
		const handleBaseTreeExpand = (rows: any[], expanded: boolean) => {
			...
			const childrenField = treeOpts.childrenField;
			const hasChildField = treeOpts.hasChildField;
			const result: any[] = [];
			...
			let validRows = toggleMethod
				? rows.filter((row: any) => toggleMethod({ $table: $xeTable, expanded, column: treeNodeColumn, columnIndex, $columnIndex, row }))
				: rows;
			if (accordion) {
		...
		const debounceScrollY = XEUtils.debounce(
			(evnt: Event) => {
		...
		tableMethods = {
			...
			getCellLabel(row, fieldOrColumn) {
				...
				if (formatter) {
					...
					if (XEUtils.isString(formatter)) {
						...
						const tcFormatMethod = gFormatOpts ? gFormatOpts.tableCellFormatMethod : null;
			...
			getCheckboxRecords(isFull) {
				...
				if (checkField) {
					if (treeConfig) {
						rowList = XEUtils.filterTree(currTableData, row => XEUtils.get(row, checkField), {
			...
			getCurrentRecord() {
				const rowOpts = computeRowOpts.value;
				return rowOpts.isCurrent ? reactData.currentRow : null;
			},
			/**
			 * 用于单选行，获取当已选中的数据
			 */
			...
			getCurrentColumn() {
				const columnOpts = computeColumnOpts.value;
				return columnOpts.isCurrent ? reactData.currentColumn : null;
			},
			/**
			 * 用于当前列，设置某列行为高亮状态
			 */
			...
			getSortColumns() {
				...
				tableFullColumn.forEach(column => {
					...
					if (column.sortable && order) {
						sortList.push({ column, field, order, sortTime: column.sortTime });
					}
				});
			...
			closeFilter() {
				...
				if (visible) {
					$xeTable.dispatchEvent(
						'filter-visible',
						{ column, property: column.field, field: column.field, filterList: $xeTable.getCheckedFilters(), visible: false },
						null,
					);
				}
				return nextTick();
			...
			setRowExpand(rows, expanded) {
				...
				if (rows) {
					...
					const validRows: any[] = toggleMethod
						? rows.filter((row: any) =>
								toggleMethod({
									$table: $xeTable,
									expanded,
			...
			setCustomStoreData(storeData: VxeTableDefines.CustomStoreData) {
				handleCustomRestore(storeData);
				this.refreshColumn(true);
				tablePrivateMethods.saveCustomStore('confirm');
			},
			focus() {
		...
		const handleGlobalMousedownEvent = (evnt: MouseEvent) => {
			...
			if (actived.row) {
				...
				if (
					!getEventTargetNode(evnt, el).flag &&
					!($xeGrid && getEventTargetNode(evnt, $xeGrid.getRefMaps().refElem.value).flag) &&
					!(tableMenu && getEventTargetNode(evnt, tableMenu.getRefMaps().refElem.value).flag) &&
					!($xeToolbar && getEventTargetNode(evnt, $xeToolbar.getRefMaps().refElem.value).flag)
				) {
		...
		const handleGlobalKeydownEvent = (evnt: KeyboardEvent) => {
			...
			if (internalData.isActivated) {
				tablePrivateMethods.preventEvent(evnt, 'event.keydown', null, () => {
					const { mouseConfig, keyboardConfig, treeConfig, editConfig } = props;
					...
					const beforeEditMethod = editOpts.beforeEditMethod;
					if (operCtxMenu) {
						...
						(selected.row || actived.row || (treeConfig && rowOpts.isCurrent && currentRow))
					) {
						// 退出选中
						if (hasCtrlKey) {
							// 如果是激活编辑状态，则取消编辑
							...
							} else if (treeConfig && rowOpts.isCurrent && currentRow) {
								// 如果是树形表格当前行回车移动到子节点
						...
						if (!isEditStatus) {
							...
							} else if ((isUpArrow || isDwArrow) && rowOpts.isCurrent) {
								// 当前行按键上下移动
					...
					} else if (hasBackspaceKey && keyboardConfig && treeConfig && keyboardOpts.isBack && rowOpts.isCurrent && currentRow) {
						// 如果树形表格回退键关闭当前行返回父节点
						const { parent: parentRow } = XEUtils.findTree(internalData.afterTreeFullData, item => item === currentRow, {
							children: childrenField,
						});
						if (parentRow) {
							evnt.preventDefault();
		...
		const handleGlobalCopyEvent = (evnt: ClipboardEvent) => {
			...
			if (isActivated && !filterStore.visible) {
				...
				tableMethods.dispatchEvent('copy', { row, column }, evnt);
			}
		};
		...
		const handleTargetEnterEvent = (isClear: boolean) => {
			...
			if (isClear) {
				...
				if ($tooltip && $tooltip.setActivated) {
		...
		tablePrivateMethods = {
					...
					if (checkField) {
						...
						if (treeConfig) {
							if (indeterminateField) {
								...
								isIndeterminate =
									!isAllSelected &&
									afterFullData.some(row => XEUtils.get(row, checkField) || !!treeIndeterminateMaps[getRowid($xeTable, row)]);
							}
						} else {
							if (indeterminateField) {
								isIndeterminate =
									!isAllSelected && afterFullData.some(row => XEUtils.get(row, checkField) || XEUtils.get(row, indeterminateField));
							} else {
					...
					if (treeConfig && !checkStrictly) {
						...
						if (matchObj && matchObj.parent) {
							...
							if (indeterminatesItem) {
								...
								parentStatus =
									selectItems.filter(item => vItemMaps[getRowid($xeTable, item)]).length === vItems.length
										? true
			...
			triggerFooterTooltipEvent(evnt, params) {
				...
				if (tooltipStore.column !== column || !tooltipStore.visible) {
					handleTooltip(evnt, cell, (cell.querySelector('.vxe-cell--item') as HTMLElement) || (cell.children[0] as HTMLElement), null, params);
				}
			},
			handleTargetLeaveEvent() {
				...
				if ($tooltip && $tooltip.setActivated) {
					$tooltip.setActivated(false);
				}
				if (tooltipOpts.enterable) {
					internalData.tooltipTimeout = setTimeout(() => {
						$tooltip = refTooltip.value;
						if ($tooltip && $tooltip.isActivated && !$tooltip.isActivated()) {
							tableMethods.closeTooltip();
						}
				...
				if (!triggerTreeNode) {
					if (!triggerExpandNode) {
						...
						if (rowOpts.isCurrent) {
							if (!triggerCheckbox && !triggerRadio) {
								...
								if (rowOpts.isCurrent && !isCheckboxType && !evnt.shiftKey) {
									tableMethods.setAllCheckboxRow(false);
									tableMethods.setCheckboxRow(row, true);
								}
							}
						}
						// 如果是单选框
						...
						// 如果是复选框
						if (!triggerCheckbox && (checkboxOpts.trigger === 'row' || ((isCheckboxType || evnt.shiftKey) && checkboxOpts.trigger === 'cell'))) {
					...
					if (isEnableConf(editConfig)) {
						...
						if (
							...
							editOpts.mode === 'cell' &&
							evnt.target &&
							/^input|textarea$/i.test((evnt.target as HTMLElement).tagName)
						) {
			...
			triggerScrollYEvent(evnt) {
				const { scrollYStore } = internalData;
				const { offsetSize, visibleSize } = scrollYStore;
				// webkit 浏览器使用最佳的渲染方式，且最高渲染量不能大于 40 条
				if (isWebkit && offsetSize * 2 + visibleSize <= 40) {
					loadScrollYData(evnt);
				} else {
			...
			updateScrollYStatus,
			// 更新横向 X 可视渲染上下剩余空间大小
			updateScrollXSpace(isUpdateAll = true) {
			...
			// 更新纵向 Y 可视渲染上下剩余空间大小
			updateScrollYSpace(isUpdateAll = true) {
				const { scrollYLoad } = reactData;
			...
			updateScrollXData() {
				...
				tablePrivateMethods.updateScrollXSpace(false);
				nextTick(() => {
					handleTableColumn();
					tablePrivateMethods.updateScrollXSpace();
				});
			...
			updateScrollYData() {
				// reactData.tableData = []
				tablePrivateMethods.updateScrollYSpace(false);
				nextTick(() => {
		...
		const renderEmptyContenet = () => {
			...
			if (slots.empty) {
				...
				const rtEmptyView = compConf ? compConf.renderTableEmpty || compConf.renderTableEmptyView : null;
				if (rtEmptyView) {
			...
			nextTick(() => {
				...
				if (props.autoResize) {
					...
					const handleOptimizeResize = refreshDelay
						? XEUtils.throttle(() => tableMethods.recalculate(true), refreshDelay, { leading: true, trailing: true })
						: null;
		...
		const renderVN = () => {
			const { loading, stripe, showHeader, height, treeConfig, mouseConfig, showFooter, editConfig, editRules } = props;
			const {
				isCalcColumn,
				isGroup,
				overflowX,
			...
			return h(
				'div',
				{
					...
					class: [
						...
						`border--${tableBorder}`,
						{
							...
							'is--group': isGroup,
							'is--tree-line': treeConfig && treeOpts.showLine,
				...
				[
					 ...
					 * 加载中
					 */
					VxeUILoadingComponent && currLoading
						? h(
					...
					initStore.filter
						? h(
								...
								{
									defer: true,
									to: computeFilterOpts.value.teleportTo,
									disabled: !computeFilterOpts.value.teleportTo,
								},
					...
					VxeUITooltipComponent
						? h('div', {}, [
								 ...
								 * 校验提示
								 */
								props.editRules && validOpts.showMessage && !height
									? h(VxeUITooltipComponent, {
											ref: refValidTooltip,
											class: ['vxe-table--valid-error'],
											...((tableData.length === 1 ? validTipOpts : {}) as any),
									  })
									: createCommentVNode(),
						  ])
						: createCommentVNode(),
