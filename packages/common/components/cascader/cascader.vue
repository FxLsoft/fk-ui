...
import { getCheckedStatus, getLeafOptionInfos, getLeafOptionKeys, getOptionInfos, getOptionLabel, getValidValues, getValueKey } from './utils';
import type { VirtualListProps } from '../_components/virtual-list-v2/interface';
import type { Size } from '../_utils/constant';
import type { CascaderFieldNames, CascaderOption, CascaderOptionInfo } from './interface';
import type { BaseType, Data, UnionType } from '../_utils/types';
...
export default defineComponent({
	...
	emits: {
		'update:modelValue': (
			value: string | number | Record<string, any> | (string | number | Record<string, any> | (string | number | Record<string, any>)[])[] | undefined,
		) => true,
	...
	setup(props, { emit, slots }) {
		const { options, checkStrictly, loadMore, formatLabel, modelValue, disabled, valueKey, expandTrigger, expandChild, pathMode, multiple } = toRefs(props);
		...
		const computedValueMap = computed(() => {
			...
			return new Map(
				// @ts-ignore
				values.map((value: UnionType | UnionType[]) => [
					getValueKey(value, {
		...
		const filteredLeafOptions = computed(() => {
			const options = props.checkStrictly ? Array.from(optionMap.values()) : Array.from(leafOptionSet);
		...
		const updateValue = (values: UnionType[] | UnionType[][]) => {
			...
			_value.value = value as any;
			emit('update:modelValue', value);
			emit('change', value);
			eventHandlers.value?.onChange?.();
		};

		const updateValueByOption = (values: CascaderOptionInfo[]) => {
			updateValue(
				values.map(v => {
					return props.pathMode ? v.pathValue : v.value;
				}),
			);
		};
		...
		watch([multiple, pathMode], () => {
			const values: any[] = [];
			computedValueMap.value.forEach((value, key: string) => {
		...
		const selectMultiple = (options: CascaderOptionInfo | CascaderOptionInfo[], checked: boolean) => {
			const option = Array.isArray(options) ? options : [options];
			if (checked) {
				let leafOptionInfos: CascaderOptionInfo[] = [];
				if (props.checkStrictly) {
					leafOptionInfos = option;
				} else {
					option.forEach(opt => leafOptionInfos.push(...getLeafOptionInfos(opt)));
				}
				updateValue([
				...
				let leafOptionKeys: string[] = [];
				if (props.checkStrictly) {
					leafOptionKeys = option.map(v => v.key);
				} else {
					option.forEach(opt => leafOptionKeys.push(...getLeafOptionKeys(opt)));
				}
				const values: any[] = [];
				computedValueMap.value.forEach((value, key: string) => {
		...
		watch(computedPopupVisible, value => {
			if (value) {
				if (computedValueMap.value.size > 0) {
					const keys: string[] = Array.from(computedValueMap.value.keys()) as string[];
		...
		const { activeKey, activeOption, selectedPath, displayColumns, setActiveKey, setSelectedPath, getNextActiveNode } = useSelectedPath(optionInfos, {
			optionMap,
			filteredLeafOptions,
			showSearchPanel,
			expandChild,
		...
		const selectViewValue = computed(() => {
			const result: any[] = [];
			computedValueMap.value.forEach((value: any, key: string) => {
				const option = leafOptionMap.get(key);
				if (option) {
					result.push({
						value: key,
