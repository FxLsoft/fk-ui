import type { InjectionKey, VNodeTypes } from 'vue';
import type { InternalMenuProps, MenuData } from './interface';
...
export type MenuContext = Readonly<
	Pick<
		InternalMenuProps,
		'mode' | 'theme' | 'levelIndent' | 'autoScrollIntoView' | 'scrollConfig' | 'inTrigger' | 'triggerProps' | 'tooltipProps' | 'popupMaxHeight'
	> & {
		selectedKeys: string[];
		openKeys: string[];
		prefixCls: string;
...
export const DataCollectorInjectionKey: InjectionKey<DataCollectorContext> = Symbol('DataCollectorInjectionKey');
