...
const monthGroup = Array.from({ length: 3 });
for (let i = 0; i < 3; i++) {
	monthGroup[i] = MONTHS.slice(i * 4, 4 * (i + 1));
}

const monthGroupPanel = Array.from({ length: 4 });
for (let i = 0; i < 4; i++) {
				...
				{mg.map((row: any[], rowIndex) => (
					<div class={`${prefixCls}-year-row`} key={rowIndex}>
						{row.map(col => {
							...
							const divProps = props.panel ? { onClick: () => props.selectHandler(time, false) } : {};

							return (
								<div key={col.value} class={getCellClassName.value({ ...col, time }, false)}>
									{props.panel ? (
										<div class={`${prefixCls}-date`} {...divProps}>
											<div class={`${prefixCls}-date-value`}>{t(`calendar.month.short.${col.name}`)}</div>
										</div>
									) : (
										<div class={`${prefixCls}-month-with-days`}>
											<div class={`${prefixCls}-month-title`}>{t(`calendar.month.long.${col.name}`)}</div>
											<Month
