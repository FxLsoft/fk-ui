...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const inkRef = shallowRef<ComponentPublicInstance>();
		...
		const getWrapperLength = () => {
			return (direction.value === 'vertical' ? wrapperRef.value?.offsetHeight : wrapperRef.value?.offsetWidth) ?? 0;
		};
		...
		const isInView = (index: number) => {
			return (tabEndOffsets.value[index - 1] ?? 0) >= offset.value && tabEndOffsets.value[index] <= offset.value + wrapperLength.value;
		};
		...
		return () => (
			<div class={cls.value}>
				{isScroll.value && <TabsButton type="previous" direction={props.direction} disabled={offset.value <= 0} onClick={handleButtonClick} />}
				...
				{isScroll.value && (
					<TabsButton type="next" direction={props.direction} disabled={offset.value >= maxOffset.value} onClick={handleButtonClick} />
				)}
