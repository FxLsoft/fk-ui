<template>
	<span :class="cls">
		...
		<input-number v-model="inputValue" :class="`${prefixCls}-input`" :min="1" :max="pages" :size="size"
			:disabled="disabled" hide-button :formatter="handleFormatter" @change="handleChange" />
		<span v-if="$slots['jumper-append']" :class="`${prefixCls}-append`">
			<slot name="jumper-append" />
		</span>
		<span v-else :class="`${prefixCls}-append`">
			/ {{ pages }} {{ t('pagination.page') }}
		</span>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const handleFormatter = (value: number) => {
			const parseIntVal = Number.parseInt(value.toString(), 10);
