...
import type { DotType, LineType, ModeType, PositionType } from './interface';
import type { Direction } from '../_utils/constant';

const getDefaultPosition = (index: number, mode: ModeType, direction: Direction, position?: string) => {
	let map = ['left', 'right'];
	...
	return map.includes(res) ? res : map[0];
};
