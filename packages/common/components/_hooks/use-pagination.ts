...
export const usePagination = (props: any, { emit }: { emit: any }) => {
	...
	const current = computed(() => (isObject(props.pagination) ? props.pagination.current ?? _current.value : _current.value));

	const pageSize = computed(() => (isObject(props.pagination) ? props.pagination.pageSize ?? _pageSize.value : _pageSize.value));
