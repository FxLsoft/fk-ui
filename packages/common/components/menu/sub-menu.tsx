...
export default defineComponent({
	...
	render() {
		const { props, attrs, computedKey, computedPopup, expandIconDown, expandIconRight, isChildrenSelected } = this;
		const _slots = {
			...this.$slots,
			'expand-icon-down': this.$slots['expand-icon-down'] || expandIconDown || (() => [<IconDown />]),
			'expand-icon-right': this.$slots['expand-icon-right'] || expandIconRight || (() => [<IconRight />]),
		};
		return computedPopup ? (
			...
			<SubMenuInline key={computedKey} title={props.title} isChildrenSelected={isChildrenSelected} v-slots={_slots} {...attrs} />
		);
	},
});
