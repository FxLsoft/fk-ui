...
export default defineComponent({
	...
	setup(props, { slots, attrs }) {
		...
		const separatorRender = () => {
			...
			const separatorElement = slots.separator?.() ?? props.separator ?? breadcrumbCtx?.slots.separator?.() ?? breadcrumbCtx?.separator ?? (
				<IconObliqueLine />
			);
			return (
		...
		const renderItem = () => {
			return (
				<div
					...
					{...attrs}
				>
					{displayMore.value ? breadcrumbCtx?.slots['more-icon']?.() ?? <IconMore /> : slots.default?.()}
		...
		const renderDropdownContent = () => {
			return slots.droplist?.() ?? props.droplist?.map(item => <Doption value={item.path}>{item.label}</Doption>);
		};
