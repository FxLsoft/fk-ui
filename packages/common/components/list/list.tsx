import { Fragment, computed, defineComponent, inject, isVNode, onMounted, ref, toRefs } from 'vue';
...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const { current, pageSize, handlePageChange, handlePageSizeChange } = usePagination(props, { emit });
					...
					items.push(
						<Grid.Row key={rowIndex} class={`${prefixCls}-row`} gutter={props.gridProps.gutter}>
							{currentPageItems.slice(i, nextIndex).map((item, index) => (
								<Grid.Col key={`${rowIndex}-${index}`} class={`${prefixCls}-col`} span={props.gridProps?.span}>
			...
			return (
				<Grid.Row class={`${prefixCls}-row`} gutter={props.gridProps.gutter}>
					{currentPageItems.map((item, index) => (
						<Grid.Col key={index} class={`${prefixCls}-col`} {...omit(props.gridProps!, ['gutter'])}>
							{isVNode(item) ? item : slots.item?.({ item, index })}
						</Grid.Col>
					))}
				</Grid.Row>
		...
		const renderListItems = (data: unknown[]) => {
			const currentPageItems = getCurrentPageItems(data);
			const list = currentPageItems.map((item, index) => (isVNode(item) ? item : slots.item?.({ item, index })));
			return list;
		};
		...
		const renderDraggableItems = () => {
			if (isObject(props.draggable)) {
				Object.assign(draggableProps, props.draggable);
			}
			const draggableSlots = {
				item: ({ element, index }) => {
					return slots.item?.({ item: element, index });
				},
			};
			return (
				<Draggable
					class={`${prefixCls}-draggable`}
					list={props.data}
					animation={300}
		...
		const renderVirtualList = () => {
			...
			return currentPageItems.length ? (
				<VirtualList
					ref={virtualListRef}
					v-slots={{
						item: ({ item, index }: { item: unknown; index: number }) => slots.item?.({ item, index }),
					}}
		...
		const renderScrollLoading = () => {
			if (slots['scroll-loading']) {
				return <div class={[`${prefixCls}-item`, `${prefixCls}-scroll-loading`]}>{slots['scroll-loading']()}</div>;
			}
			return null;
		};
		...
		const render = () => {
			const Component: any = displayScrollbar.value ? Scrollbar : 'div';
			return (
				<div class={`${prefixCls}-wrapper`}>
					<Spin class={`${prefixCls}-spin`} loading={props.loading}>
						<Component ref={componentRef} class={cls.value} style={contentStyle.value} {...scrollbarProps.value} onScroll={handleScroll}>
