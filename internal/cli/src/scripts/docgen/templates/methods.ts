...
const tmpl = (methods: MethodDescriptor[], lang: string) => {
	const displayableMethods = methods.filter(method => method.description || lang in (method.tags ?? {}));
	...
	const content = displayableMethods
		.map(method => {
			...
			let lineContent = `|${name}|${escapeCharacter(description || '')}|${escapeCharacter(readableParams)}|${escapeCharacter(readableReturns)}|`;
...
export default (methods: MethodDescriptor[], lang: string) => {
	...
	const header = lang === 'en' ? ['|Method|Description|Parameters|Return|', '|---|---|---|:---:|'] : ['|方法名|描述|参数|返回值|', '|---|---|---|---|'];
