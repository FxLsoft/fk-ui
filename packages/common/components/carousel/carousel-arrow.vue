<template>
	<div :class="cls">
		<div :class="`${prefixCls}-arrow-${direction === 'vertical' ? 'top' : 'left'}`" @click="onPreviousClick">
			<IconLeft v-if="direction === 'horizontal'" />
			<IconUp v-else />
		</div>
		<div :class="`${prefixCls}-arrow-${direction === 'vertical' ? 'bottom' : 'right'}`" @click="onNextClick">
