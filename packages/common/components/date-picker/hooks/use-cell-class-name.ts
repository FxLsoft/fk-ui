...
import type { Cell, IsSameTime, Mode } from '../interface';
...
export default function useCellClassName(props: CellClassNameProps) {
	...
	const sortedRangeValues = computed(() =>
		rangeValues?.value && rangeValues.value.every(isDayjs) ? getSortedDayjsArray(rangeValues.value) : rangeValues?.value
	);
	...
	return {
		getCellClassName: (cellData: Cell, disabled: boolean) => {
			...
			const isInRange =
				...
				rangeStart.value &&
				rangeEnd.value &&
				(isRangeStart || isRangeEnd || cellData.value.isBetween(rangeStart.value, rangeEnd.value, null, '[]'));
