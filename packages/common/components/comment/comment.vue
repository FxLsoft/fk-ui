	...
	<div :class="prefixCls">
		...
		<div :class="`${prefixCls}-inner`">
			<div :class="`${prefixCls}-inner-content`">
				<div v-if="hasAuthor || hasDatetime" :class="`${prefixCls}-title ${prefixCls}-title-align-${computedAlign.datetime}`">
				...
				<div v-if="$slots.actions" :class="`${prefixCls}-actions ${prefixCls}-actions-align-${computedAlign.actions}`">
					<slot name="actions" />
				</div>
			</div>
...
export default defineComponent({
	...
	setup(props, { slots }) {
		...
		const [hasAuthor, hasAvatar, hasContent, hasDatetime] = ['author', 'avatar', 'content', 'datetime'].map(propName =>
			hasPropOrSlot(props, slots, propName),
		);
