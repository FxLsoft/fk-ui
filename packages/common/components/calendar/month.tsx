...
export function getAllDaysByTime(time: Dayjs, { dayStartOfWeek = 0, isWeek }: { dayStartOfWeek: 0 | 1; isWeek: boolean }) {
	const current = getTimeObj(time);

	const flatRows = Array.from({ length: allDaysInOnePage })
		.fill(null)
		.map(() => ({}));
							...
							{slots.default ? (
								...
								<div class={`${prefixCls}-date`} {...tdDivProps}>
									<div class={`${prefixCls}-date-value`}>
										{props.panel ? col.date : <div class={`${prefixCls}-date-circle`}>{col.date}</div>}
									</div>
								</div>
							)}
						</div>
