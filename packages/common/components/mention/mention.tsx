import { computed, defineComponent, nextTick, onMounted, ref, toRef, toRefs, watch } from 'vue';
...
export default defineComponent({
	...
	emits: {
		 ...
		 */
		select: (value: string | number | boolean | Record<string, unknown>) => true,
		/**
		 * @zh 用户点击清除按钮时触发
	...
	setup(props, { emit, attrs, slots }) {
		...
		const renderDropdown = () => {
			return <SelectDropdown ref={dropdownRef}>{validOptions.value.map(info => renderOption(info as SelectOptionInfo))}</SelectDropdown>;
		};
