...
import type { Key2TreeNode, TreeNodeKey } from '../interface';
...
export default function useCheckedState(props: {
	...
	const { defaultCheckedKeys, checkedKeys: propCheckedKeys, key2TreeNode, checkStrictly, halfCheckedKeys, onlyCheckLeaf } = toRefs(props);
	...
	return {
		...
		setCheckedState(newCheckedKeys: TreeNodeKey[], newIndeterminateKeys: TreeNodeKey[], reinitialize = false) {
			if (reinitialize) {
				init(newCheckedKeys);
			} else {
