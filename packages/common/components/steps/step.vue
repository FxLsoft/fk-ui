...
<script lang="ts">
import { computed, defineComponent, getCurrentInstance, inject, onBeforeUnmount, reactive, ref, watch } from 'vue';
import { getPrefixCls } from '../_utils/global-config';
import IconCheck from '../icon/icon-check';
import IconClose from '../icon/icon-close';
import { Direction } from '../_utils/constant';
import { useIndex } from '../_hooks/use-index';
import { StepsType } from './interface';
...
export default defineComponent({
	...
	setup(props) {
		...
		const computedStatus = computed(() => props.status ?? stepsCtx?.getStatus(stepNumber.value) ?? 'process');

		const nextStepError = computed(() => stepsCtx?.errorSteps.includes(stepNumber.value + 1) ?? false);
		...
		const showTail = computed(() => !stepsCtx?.lineLess && (stepsCtx?.labelPlacement === 'vertical' || stepsCtx?.direction === 'vertical'));
