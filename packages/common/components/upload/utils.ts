...
import type { FileItem, FileStatus, RequestOption, UploadRequest } from './interfaces';
	...
	if (accept && file) {
		...
		return accepts.some(type => {
			...
			if (/\..*/.test(typeText)) {
				...
				return suffixList.includes(fileExtension);
			}
			return false;
		});
	}
...
export const loopDirectory = (itemList: DataTransferItemList, accept: string | undefined, callback: (files: File[]) => void) => {
	const files: File[] = [];
	...
	Array.prototype.slice.call(itemList).forEach((item: DataTransferItem) => item.webkitGetAsEntry && _loopDirectory(item.webkitGetAsEntry()));
};
