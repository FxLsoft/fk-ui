	...
	setup(props, { emit, slots }) {
		...
		const onResize = () => {
			if (listRef.value && children.value && spacerRef.value) {
				...
				let remainingWidth = listRef.value.clientWidth - (overflowRef.value?.offsetWidth ?? 0) - (isStart.value ? props.margin : 0);
		...
		const renderOverflow = (overflowNodes: VNode[]) => {
			const style = isStart.value ? { marginRight: `${props.margin}px` } : undefined;

			return (
				<Tooltip v-slots={{ content: () => overflowNodes }}>
		...
		return () => {
			...
			if (overflowNumber.value > 0) {
				...
				overflowChildren = isStart.value ? children.value.slice(0, overflowNumber.value) : children.value.slice(-overflowNumber.value);
			}
			...
			for (let i = 0; i < overflowChildren.length - 1; i++) {
				const el = overflowChildren[i];
				el.props = mergeProps(el.props ?? {}, {
					style: { margin: `${props.margin / 2}px` },
				});
			}

			return (
				<div ref={listRef} class={prefixCls}>
					{isStart.value && overflowNumber.value > 0 && renderOverflow(overflowChildren)}
					{visibleChildren}
					{!isStart.value && overflowNumber.value > 0 && renderOverflow(overflowChildren)}
