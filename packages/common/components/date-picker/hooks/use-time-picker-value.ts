...
import type { TimePickerProps, TimeValue } from '../../time-picker/interface';
...
export default function useTimePickerValue(props: TimePickerValueProps): [Ref<Dayjs>, (val: Dayjs | undefined) => void, () => void] {
	...
	const timePickerPropsUse12Hours = computed(() => !!timePickerProps?.value?.use12Hours);
	...
	const defaultTimePickerValue = computed(() => getDayjsValue(timePickerProps?.value?.defaultValue as TimeValue | undefined, format.value));

	const getDefaultValue = () => selectedValue?.value || defaultTimePickerValue.value || getNow();
