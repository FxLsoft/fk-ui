...
const jsongen = async () => {
	...
	await fs.writeFile(path.resolve(process.cwd(), 'json/vetur-tags.json'), JSON.stringify(tags, null, 2));
	await fs.writeFile(path.resolve(process.cwd(), 'json/vetur-attributes.json'), JSON.stringify(attributes, null, 2));

	const web = transformToWebTypes(docs, { version: packageData.version });

	await fs.writeFile(path.resolve(process.cwd(), 'json/web-types.json'), JSON.stringify(web, null, 2));
};
