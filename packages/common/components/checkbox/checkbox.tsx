...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const checkboxGroupCtx = !props.uninjectGroupContext ? inject(checkboxGroupKey, undefined) : undefined;
		const isGroup = checkboxGroupCtx?.name === 'CheckboxGroup';
		const { mergedDisabled: _mergedDisabled, eventHandlers } = useFormItem({
			disabled,
		});

		const _checked = ref(props.defaultChecked);
		const computedValue = computed(() => (isGroup ? checkboxGroupCtx?.computedValue : props.modelValue ?? _checked.value));
		const computedChecked = computed<boolean>(() => {
			return isArray(computedValue.value) ? computedValue.value.includes(props.value ?? true) : computedValue.value;
		});
		const mergedDisabled = computed(() => checkboxGroupCtx?.disabled || _mergedDisabled?.value || (!computedChecked.value && checkboxGroupCtx?.isMaxed));
		...
		const handleChange = (e: Event) => {
			...
			if (isGroup && isArray(newValue)) {
				if (checkboxGroupCtx.multiple) {
					checkboxGroupCtx?.handleChange(newValue, e);
				} else {
					if (checked) {
						checkboxGroupCtx?.handleChange([props.value], e);
					} else {
						checkboxGroupCtx?.handleChange([], e);
					}
				}
			} else {
		...
		return () => (
			<label aria-disabled={mergedDisabled.value} class={cls.value}>
				...
				{(slots.checkbox ?? checkboxGroupCtx?.slots?.checkbox)?.({
					...
					<IconHover class={`${prefixCls}-icon-hover`} disabled={mergedDisabled.value || computedChecked.value}>
						<div class={`${prefixCls}-icon`}>{computedChecked.value && <IconCheck class={`${prefixCls}-icon-check`} />}</div>
					</IconHover>
				)}
				{(slots.default || props.label) && <span class={`${prefixCls}-label`}>{props.label ?? slots.default()}</span>}
			</label>
		);
	},
});
