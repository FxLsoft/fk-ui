...
function getSizeConfig(size: number | string) {
	const numberSize = isString(size) ? Number.parseFloat(size) : size;
	let unit = '';
...
function getPxSize({ size, defaultSize, containerSize }: { size: number | string | undefined; defaultSize?: string; containerSize: number }) {
...
function px2percent(numerator: number | string, denominator: number | string) {
	return Number.parseFloat(numerator as string) / Number.parseFloat(denominator as string);
}
