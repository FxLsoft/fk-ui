import { computed, defineComponent, onMounted, onUnmounted, onUpdated, reactive, ref, shallowRef, toRefs, watch } from 'vue';
...
import type { PropType, VNode, VNodeTypes } from 'vue';
...
export default defineComponent({
	...
	setup(props, { slots, emit, attrs }) {
		...
		const ellipsisConfig = computed<EllipsisInternalConfig>(() => normalizeEllipsisConfig((isObject(ellipsis.value) && ellipsis.value) || {}));
		...
		function calEllipsis() {
			...
			const { ellipsis, text } = measure(wrapperRef.value, ellipsisConfig.value, renderOperations(!!ellipsisConfig.value.expandable), fullText.value);
		...
		return () => {
			...
			const { suffix, ellipsisStr, showTooltip, tooltipProps, TooltipComponent } = ellipsisConfig.value;
			...
			if (ellipsisConfig.value.css) {
				...
				const Outer = (
					<Component class={classNames.value} ref={wrapperRef} style={ellipsisStyle.value} {...titleAttrs} {...attrs}>
