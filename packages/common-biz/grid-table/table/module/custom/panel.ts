import { TransitionGroup, createCommentVNode, defineComponent, h, inject, ref } from 'vue';
...
export default defineComponent({
	...
	setup(props) {
		...
		const VxeUIDrawerComponent = VxeUI.getComponent('Drawer');
		...
		const VxeUIRadioGroupComponent = VxeUI.getComponent('RadioGroup');
		...
		const sortDragstartEvent = (evnt: DragEvent) => {
			...
			try {
				// ie throw error
				// firefox-need-it
				evnt.dataTransfer?.setData('text/plain', '');
			} catch {
				// empty
			}
		};
		...
		const updateDropHint = (evnt: DragEvent) => {
			...
			if (dragHintEl) {
				...
				dragHintEl.style.left = `${Math.min(
					wrapperEl.clientWidth - wrapperEl.scrollLeft - dragHintEl.clientWidth - 16,
					evnt.clientX - wrapperRect.x,
				)}px`;
			}
		};
											...
											[
												...
												VxeUIButtonComponent
													? h(VxeUIButtonComponent, {
															...
															// type: column.renderFixed === 'right' ? 'primary' : '',
															disabled: isHidden || (isMaxFixedColumn && !column.renderFixed),
															title: getI18n(
								...
								? h(
										...
										[
											VxeUIButtonComponent
												? h(VxeUIButtonComponent, {
														type: 'text',
														label: customOpts.resetButtonText || getI18n('vxe.table.customRestore'),
											...
											customOpts.immediate
												? VxeUIButtonComponent
													? h(VxeUIButtonComponent, {
															type: 'text',
															label: customOpts.closeButtonText || getI18n('vxe.table.customClose'),
												...
												? h(VxeUIButtonComponent, {
														type: 'text',
														label: customOpts.cancelButtonText || getI18n('vxe.table.customCancel'),
											...
											customOpts.immediate
												...
												? h(VxeUIButtonComponent, {
														type: 'text',
														label: customOpts.confirmButtonText || getI18n('vxe.table.customConfirm'),
