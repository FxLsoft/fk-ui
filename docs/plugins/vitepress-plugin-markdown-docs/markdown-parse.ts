...
export function getHash(content: string, type = 'md5', encoding = 'utf8') {
	return crypto
		.createHash(type)
		.update(content, encoding as Encoding)
		.digest('hex');
}
...
export function injectImports(content: string, modules: VirtualModule[]) {
	...
	const improts = modules
		.map(({ componentName, virtualPath }) => {
...
export function parseCodeBlock(source: string, handler: (virtualPath: string, code: string) => void) {
	...
	const content = source.replace(re, (_, $1, $2) => {
		...
		try {
			...
			console.log('md-vue-sfc-plugin SFCBlockConfig parse error', e, source);
			return _;
		}
		...
		const detailCode = `\n::: details 代码事例 \n\`\`\`vue\n${$2}\n\`\`\`\n:::`;
		return config.lazy ? '' : `\n<ClientOnly><${componentName} /></ClientOnly>${detailCode}`;
	});
...
export function createPlugin() {
	...
	const fileIncludes = new Map<string, string[]>();

	function parseFile(source: string, id: string) {
		const includes: string[] = [];
		source = processIncludes(process.cwd(), source, id, includes);
		fileIncludes.set(id, includes);
