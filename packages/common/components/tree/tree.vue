<template>
	<VirtualList v-if="virtualListProps" ref="virtualListRef" :class="classNames" v-bind="virtualListProps" :data="visibleTreeNodeList">
...
<script lang="ts">
import { computed, defineComponent, provide, reactive, ref, shallowRef, toRefs, watch } from 'vue';
...
import type { CheckedStrategy, DropPosition, FilterTreeNode, Node, TreeFieldNames, TreeNodeData, TreeNodeKey } from './interface';
...
export default defineComponent({
	...
	props: {
		 ...
		 * */
		allowDrop: {
			type: Function as PropType<(options: { dropNode: TreeNodeData; dropPosition: -1 | 0 | 1 }) => boolean>,
		},
		/**
		 * @zh 过滤节点
		 * @en Filter node
		...
		/**
		 * @zh 默认搜索时展开所有节点
		 * @en Expand all nodes when searching by default
		 */
		defaultExpandAllBySearch: {
			type: Boolean,
			default: true,
		},
	},
	emits: {
		 ...
		 */
		drop: (data: { e: DragEvent; dragNode: TreeNodeData; dropNode: TreeNodeData; dropPosition: number }) => true,
	},
	/**
	 * @zh 定制节点图标
	 * @en Custom node icon
	 ...
	 * @slot extra
	 * @binding {string} data 是数据模型  nodeStatus 节点状态(响应式，直接操作节点状态)   node 是节点所有方法和属性(不建议使用，破坏性大) 类型为 Node
	 */
	/**
	 * @zh 标题
	 * @en Title
	...
	setup(props, { emit, slots }) {
		...
		const { treeData, flattenTreeData, key2TreeNode, filterTreeData } = useTreeData(
			reactive({
		...
		function getDefaultExpandedKeys() {
			if (defaultExpandedKeys?.value) {
				...
				defaultExpandedKeys.value.forEach(_key => {
					...
					[...(autoExpandParent.value ? node.pathParentKeys : []), _key].forEach(_key => expandedKeysSet.add(_key));
				});
		...
		const visibleTreeNodeList = computed(() => {
			const expandedKeysSet: Set<TreeNodeKey> = new Set(expandedKeys.value);
			const currentExpandKeysSet = new Set(currentExpandKeys.value);
			return filterTreeData.value.filter(node => {
				const isRoot = isUndefined(node.parentKey);

				const isVisibleNode = node.pathParentKeys?.every(_key => expandedKeysSet.has(_key) && !currentExpandKeysSet.has(_key));

				return isRoot || isVisibleNode;
			});
		});

		function getPublicCheckedKeys(rawCheckedKeys: TreeNodeKey[], rawCheckedStrategy = checkedStrategy.value) {
		...
		function getNodes(keys: TreeNodeKey[]) {
			return keys.map(key => key2TreeNode.value.get(key)?.treeNodeData || undefined).filter(Boolean);
		}
		...
		function emitSelectEvent(options: { targetKey?: TreeNodeKey; targetSelected?: boolean; newSelectedKeys: TreeNodeKey[]; event?: Event }) {
		...
		function emitExpandEvent(options: { targetKey?: TreeNodeKey; targetExpanded?: boolean; newExpandedKeys: TreeNodeKey[]; event?: Event }) {
		...
		function onCheck(checked: boolean, key: TreeNodeKey, e?: Event) {
			...
			const [newCheckedKeys, newIndeterminateKeys] = getCheckedStateByCheck({
				...
				checkStrictly: checkStrictly.value,
				filterNodes: filterTreeData.value,
			});
			setCheckedState(newCheckedKeys, newIndeterminateKeys);
			emitCheckEvent({
				targetKey: key,
		...
		function onSelect(key: TreeNodeKey, e: Event) {
			...
			if (multiple.value) {
				...
				if (props.onlyCheckLeaf && node.children?.length && !checkable.value) {
					onExpand(!expandedKeys.value.includes(key), key, e);
					return;
				}
				selected = true;
				newSelectedKeys = [key];
		...
		watch(
			() => !!props.searchValue,
			value => {
				if (!props.defaultExpandAllBySearch) {
					return;
				}
				if (value) {
					setExpandedKeys(flattenTreeData.value.filter(node => node.children && node.children.length).map(node => node.key));
				} else {
		...
		// 	value => {
		// 		if (value) {
		// 			setExpandState(getDefaultExpandedKeys());
		// 		} else {
		// 			setExpandState(propExpandedKeys.value ?? getDefaultExpandedKeys());
		// 		}
		// 	},
		// );
	...
	methods: {
		 ...
		 * @public
		 * @version 1.0.0，onlyCheckLeaf from 2.21.0
		 */
