 ...
 * 视口已滚动距离 / 总可滚动距离
 */
export function getScrollPercentage({ scrollTop, scrollHeight, clientHeight }: { scrollTop: number; scrollHeight: number; clientHeight: number }) {
 ...
 * 计算元素相对于视口顶部的偏移量
 */
export function getItemRelativeTop({ itemHeight, itemOffsetPtg, scrollPtg, clientHeight }: Omit<ItemTopConfig, 'scrollTop'>) {
