	...
	<div :class="`${prefixCls}-header`">
		...
		<div :class="`${prefixCls}-header-title`">
			<template v-if="onLabelClick && (year || month)">
				<span v-if="year" :class="`${prefixCls}-header-label`" @click="() => onLabelClick && onLabelClick('year')">{{ year }}</span>
				<span v-if="year && month">-</span>
				<span v-if="month" :class="`${prefixCls}-header-label`" @click="() => onLabelClick && onLabelClick('month')">{{ month }}</span>
			</template>
...
import type { HeaderIcons, Mode } from '../interface';
