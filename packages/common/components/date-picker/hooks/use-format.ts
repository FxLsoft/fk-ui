...
import type { FormatFunc, Mode, ValueFormat } from '../interface';
...
export default function useFormat(props: FormatProps) {
	...
	const format = computed(() => (!isFunction(propFormat?.value) && propFormat?.value) || getDefaultFormat(mode?.value, showTime?.value));

	const valueFormat = computed(() => propValueFormat?.value || getDefaultValueFormat(mode?.value, showTime?.value));
