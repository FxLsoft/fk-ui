...
<script lang="ts">
import { computed, defineComponent, nextTick, reactive, ref, shallowRef, toRefs, watch } from 'vue';
import { dayjs, getDateValue, getSortedDayjsArray, isValueChange } from '../_utils/date';
...
export default defineComponent({
	...
	props: {
		 ...
		 * */
		modelValue: {
			type: [String, Number, Date, Array] as PropType<string | number | Date | Array<string | number | Date>>,
		},
		/**
		 * @zh 默认值
		 * @en Default value
	...
	setup(props, { emit }) {
		...
		const { computedValue, panelValue, inputValue, setValue, setPanelValue, setInputValue } = useTimeState(
			reactive({
				modelValue,
				defaultValue,
				isRange,
		...
		function onRangeInputChange(e: any) {
			...
			const newInputValue = isArray(inputValue.value)
				? [...inputValue.value]
				: (isArray(panelValue.value) && (getFormattedValue(panelValue.value, computedFormat.value) as Array<string | undefined>)) || [];
