<template>
	...
	>
		...
		<div v-if="$slots['trigger-icon']" :class="`${prefixCls}-trigger-icon-${triggerType}`" :style="computedTriggerIconStyle">
...
<script lang="ts">
import { computed, defineComponent, inject, nextTick, onMounted, ref, toRefs, watch } from 'vue';
...
import type { AvatarShape, AvatarTriggerType, ObjectFit } from './interface';
...
export default defineComponent({
	...
	setup(props, { slots, emit, attrs }) {
		...
		onMounted(() => {
			if (wrapperRef.value?.firstElementChild && ['IMG', 'PICTURE'].includes(wrapperRef.value.firstElementChild.tagName)) {
				isImage.value = true;
			}
