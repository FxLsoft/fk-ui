...
import type { PreviewGroupContext } from './context';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const propImageUrlMap = computed(() => new Map(isArray(srcList?.value) ? srcList?.value.map((url, index) => [index, { url, canPreview: true }]) : []));
		...
		return {
			...
			groupProps: reactive({
				...
				currentId,
				setCurrentId,
				previewUrlFormatter: props.previewUrlFormatter,
			}),
		};
	},
});
