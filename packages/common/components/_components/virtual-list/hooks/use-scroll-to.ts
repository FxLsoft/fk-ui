...
import { getCompareItemRelativeTop, getItemRelativeTop, getRangeIndex, getScrollPercentage } from '../utils';
import type { InternalDataItem, ScrollOptions, VirtualItemKey } from '../interface';
...
export function useScrollTo(props: {
	...
	const { isVirtual, isStaticItemHeight, data, rangeState, viewportRef, scrollTop, visibleCount, getItemHeightOrDefault, getItemHeightOrDefaultByIndex } =
		toRefs(props);
	...
	const prepareScrollTo = (options: ScrollOptions) => {
		...
		const index = 'index' in options ? (options.index as number) : 'key' in options ? data.value.findIndex(item => item.key === options.key) : 0;
