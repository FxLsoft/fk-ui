import { computed, defineComponent, isVNode, provide, reactive, toRefs } from 'vue';
...
import type { DescData, DescItemData, RenderData } from './interface';
...
export default defineComponent({
	...
	setup(props, { slots }) {
		...
		const labelAlign = computed(
			() => (isObject(props.align) ? (props.align as { label?: TextAlign; value?: TextAlign }).label : (props.align as TextAlign)) ?? 'left',
		);
		const valueAlign = computed(
			() => (isObject(props.align) ? (props.align as { label?: TextAlign; value?: TextAlign }).value : (props.align as TextAlign)) ?? 'left',
		);
		...
		const getGroupedData = (data: (DescData | VNode)[]) => {
			...
			data.forEach((item, index) => {
				const itemSpan = Math.min((isVNode(item) ? sortedSpans.value[index] : item.span) ?? 1, computedColumn.value);
		...
		const renderLabel = (item: DescData | VNode, index: number) => {
			...
			return slots.label?.({ label: item.label, index, data: item }) ?? (isFunction(item.label) ? item.label() : item.label);
		};
		...
		const renderValue = (item: DescData | VNode, index: number) => {
			...
			return slots.value?.({ value: item.value, index, data: item }) ?? (isFunction(item.value) ? item.value() : item.value);
		};
		...
		const renderHorizontalItems = (data: RenderData[], index: number) => (
			<tr class={`${prefixCls}-row`} key={`tr-${index}`}>
				{data.map(item => (
					<>
						<td class={[`${prefixCls}-item-label`, `${prefixCls}-item-label-block`]} style={labelStyle.value}>
							{renderLabel(item.data, index)}
						</td>
						<td class={[`${prefixCls}-item-value`, `${prefixCls}-item-value-block`]} style={valueStyle.value} colspan={item.span * 2 - 1}>
							{renderValue(item.data, index)}
						</td>
					</>
				))}
			...
			<tr class={`${prefixCls}-row`} key={`inline-${index}`}>
				{data.map((item, index) => (
					<td key={`item-${index}`} class={`${prefixCls}-item`} colspan={item.span}>
						<div class={[`${prefixCls}-item-label`, `${prefixCls}-item-label-inline`]} style={labelStyle.value}>
							{renderLabel(item.data, index)}
						</div>
						<div class={[`${prefixCls}-item-value`, `${prefixCls}-item-value-inline`]} style={valueStyle.value}>
							{renderValue(item.data, index)}
						</div>
					</td>
				))}
