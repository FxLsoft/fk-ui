...
export default defineComponent({
	...
	props: {
		...
		placeholder: String,
		/**
		 * @zh 输入值的最大长度，errorOnly 属性在 1.0.0 版本添加
		 * @en Enter the maximum length of the value, the errorOnly attribute was added in version 1.0.0
		 */
		maxLength: {
	...
	setup(props, { emit, slots, attrs }) {
		...
		const { mergedSize: _mergedSize, mergedDisabled, mergedError: _mergedError, feedback, eventHandlers } = useFormItem({ size, disabled, error });
		...
		const mergedError = computed(
			() => _mergedError.value || Boolean(isObject(props.maxLength) && props.maxLength.errorOnly && valueLength.value > maxLength.value),
		);
		...
		const emitChange = (value: string, ev: Event) => {
			if (value !== preValue) {
				console.log('emitChange >>', preValue, value);
				preValue = value;
		...
		const renderInput = (hasOuter?: boolean) => (
			<span class={wrapperCls.value} onMousedown={handleMousedown} {...(!hasOuter ? wrapperAttrs.value : undefined)}>
				...
				/>
				{showClearBtn.value && (
					<IconHover prefix={prefixCls} class={`${prefixCls}-clear-btn`} onClick={handleClear}>
						<IconClose />
					</IconHover>
				)}
				{(slots.suffix || (Boolean(props.maxLength) && props.showWordLimit) || Boolean(feedback.value)) && (
					<span class={[`${prefixCls}-suffix`, { [`${prefixCls}-suffix-has-feedback`]: feedback.value }]}>
			...
			if (slots.prepend || slots.append) {
				return (
					<span class={outerCls.value} {...wrapperAttrs.value}>
						{slots.prepend && <span class={`${prefixCls}-prepend`}>{slots.prepend()}</span>}
						{renderInput(true)}
						{slots.append && <span class={`${prefixCls}-append`}>{slots.append()}</span>}
					</span>
				);
			}
			return renderInput();
		...
		const syncValue = () => {
			nextTick(() => {
				preValue = props.modelValue ?? _value.value;
			});
		};
