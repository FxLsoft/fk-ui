		...
		<div :class="`${prefixCls}-header`">
			...
			>
				<span :class="`${prefixCls}-header-title`">
					<span v-if="allowClear || simple || !showSelectAll" :class="`${prefixCls}-header-title-simple`">
						{{ title }}
					</span>
					<Checkbox
						v-else
				...
				<icon-hover v-if="allowClear" :disabled="disabled" :class="`${prefixCls}-header-clear-btn`" @click="handleClear">
					<icon-delete />
				</icon-hover>
				<span v-else-if="!simple" :class="`${prefixCls}-header-count`"> {{ dataInfo.selected.length }} / {{ dataInfo.data.length }} </span>
			</slot>
		</div>
		<div v-if="showSearch" :class="`${prefixCls}-search`">
			<input-search v-model="filter" :disabled="disabled" v-bind="inputSearchProps" @change="handleSearch" />
		</div>
		<div :class="`${prefixCls}-body`">
			<Scrollbar v-if="filteredData.length > 0">
				<slot :data="filteredData" :selected-keys="transferCtx?.selected" :on-select="transferCtx?.onSelect">
					<List
						:data="filteredData"
...
import type { DataInfo, TransferItem } from './interface';
...
export default defineComponent({
	...
	props: {
		...
		draggable: [Boolean, Object] as PropType<boolean | DraggableProps>,
	},
	emits: ['search'],
	setup(props, { emit }) {
		...
		const filteredData = computed(() => {
			return props.dataInfo.data
				.filter((item, index) => {
					if (!Object.hasOwnProperty.call(item, 'sort')) {
						item.sort = index;
					}
					if (filter.value) {
				...
				.sort((x, y) => {
					return x.sort - y.sort > 0 ? 1 : -1;
				});
		});

		const handleDragEnd = (evt: { oldIndex: number; newIndex: number }) => {
			filteredData.value.forEach((el, index) => {
				el.sort = index;
			});
		};
