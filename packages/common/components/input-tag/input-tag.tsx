import { TransitionGroup, computed, defineComponent, nextTick, onMounted, reactive, ref, toRefs, watch } from 'vue';
...
import { getElementStyle } from '../_utils/dom';
import { getValueData } from './utils';
import type { InputTagFieldNames, TagData } from './interface';
...
export default defineComponent({
	...
	emits: {
		 ...
		 * @param {Event} ev
		 */
		tagClick: (value: string | number | TagData, ev: Event) => true,
	},
	/**
	 * @zh 后缀元素
	 * @en Suffix
	...
	setup(props, { emit, slots, attrs }) {
		...
		const maxTagCount = ref(props.maxTagCount);
		...
		const tags = computed(() => {
			if (maxTagCount.value > 0) {
				const invisibleTags = valueData.value.slice(maxTagCount.value);
				if (invisibleTags.length > 0) {
					const result = valueData.value.slice(0, maxTagCount.value);
					const raw = {
						value: `__${prefixCls}__more`,
						label: `+${invisibleTags.length}`,
						closable: false,
						tags: invisibleTags,
					};
					result.push({
			...
			return valueData.value.slice(0, 20);
		});
		...
		const handleMoreRemove = (value: string | number, index: number, e: Event) => {
			const newValue = computedValue.value?.filter(el => {
				if (isObject(el)) {
		...
		const initTagView = throttleByRaf(async () => {
			...
			const inner = rootRef.value.querySelector(`.${prefixCls}-inner`) as HTMLElement;
			if (!inner) {
				return;
			}
			...
			const viewWidth = inner.offsetWidth;

			const tags = inner.querySelectorAll<HTMLElement>(`.${prefixCls}-tag`);
			let inputWidth = inputRef.value.getBoundingClientRect().width;
			if (inputWidth > 30) {
				inputWidth = 30;
			}
			for (let i = 0; i < tags.length; i++) {
				...
				// const ml = Number.parseInt(getElementStyle(tag, 'marginLeft'), 10);
				const mr = Number.parseInt(getElementStyle(tag, 'marginRight'), 10);
				const offsetLeft = tag.offsetLeft + rect.width + mr;
				if (offsetLeft > viewWidth - inputWidth) {
					if (i <= 0) {
						maxTagCount.value = 1;
					} else {
						maxTagCount.value = i;
		...
		const render = () => (
			<span ref={rootRef} class={cls.value} onMousedown={handleMousedown} {...wrapperAttrs.value}>
				...
				<TransitionGroup
					...
					onAfterEnter={initTagView}
					onAfterLeave={initTagView}
				>
					{tags.value.map((item, index) => (
						<Tag
							...
							onClick={evt => emit('tagClick', item.value, evt)}
							{...item.tagProps}
							...
							v-slots={{
								tip: () =>
									item.tags?.map(t => (
										<Tag
											key={`__tag-${t.value}`}
											closable={!mergedDisabled.value && !props.readonly && t.closable}
											class={`${prefixCls}-tooltip-tag`}
											nowrap={false}
											onClick={evt => emit('tagClick', t.value, evt)}
											onClose={(ev: MouseEvent) => handleMoreRemove(t.value, index, ev)}
										>
											{t.label}
										</Tag>
									)) || item.label,
