<template>
	<div v-bind="getWrapperAttrs($attrs)" :class="wrapperCls" @mousedown="handleMousedown">
		...
		<slot name="suffix" />
		<div v-if="computedMaxLength && showWordLimit" :class="`${prefixCls}-word-limit`">{{ valueLength }}/{{ computedMaxLength }}</div>
...
import type { AutoSize } from './interface';
...
export default defineComponent({
	...
	props: {
		 ...
		 * @en Maximum length of input value, the errorOnly attribute was added in version 1.0.0
		 */
		maxLength: {
	...
	setup(props, { emit, attrs }) {
		...
		const { mergedDisabled, mergedError: _mergedError, eventHandlers } = useFormItem({ disabled, error });

		const textareaRef = shallowRef<HTMLInputElement>();
		const textareaStyle = ref<CSSProperties>();
		const mirrorRef = shallowRef<HTMLInputElement>();
		...
		const mergedError = computed(
			() => _mergedError.value || Boolean(computedMaxLength.value && maxLengthErrorOnly.value && valueLength.value > computedMaxLength.value),
		);
		...
		const updateValue = (value: string, inner = true) => {
			if (computedMaxLength.value && !maxLengthErrorOnly.value && getValueLength(value) > computedMaxLength.value) {
				value = props.wordSlice?.(value, computedMaxLength.value) ?? value.slice(0, computedMaxLength.value);
			}
		...
		const minHeight = computed(() => {
			if (!isObject(props.autoSize) || !(props.autoSize as AutoSize).minRows) {
				return 0;
			}
			return (props.autoSize as AutoSize).minRows * lineHeight.value + outerHeight.value;
		});
		const maxHeight = computed(() => {
			if (!isObject(props.autoSize) || !(props.autoSize as AutoSize).maxRows) {
				return 0;
			}
			return (props.autoSize as AutoSize).maxRows * lineHeight.value + outerHeight.value;
		});
