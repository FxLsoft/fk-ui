import { computed, defineComponent, nextTick, ref, shallowRef, toRefs, watch, watchEffect } from 'vue';
import { getPrefixCls } from '../_utils/global-config';
import { isArray, isBoolean, isEmptyObject, isFunction, isNull, isNumber, isObject, isString, isUndefined } from '../_utils/is';
...
import type { OptionValueWithKey, SelectFieldNames, SelectOptionData, SelectOptionGroup, SelectOptionGroupInfo, SelectOptionInfo } from './interface';
...
export default defineComponent({
	...
	props: {
		 ...
		 */
		fallbackOption: {
			type: [Boolean, Function] as PropType<boolean | ((value: string | number | boolean | Record<string, unknown>) => SelectOptionData)>,
			default: true,
		},
		/**
		 * @zh 是否在下拉菜单中显示额外选项
	...
	setup(props, { slots, emit, attrs }) {
		...
		const retainInputValue = computed(() => isObject(props.allowSearch) && Boolean((props.allowSearch as any).retainInputValue));

		// refs
		const dropdownRef = shallowRef<ComponentPublicInstance>();
		const selectViewRef = shallowRef<InstanceType<typeof SelectView>>();
		const optionRefs = ref<Record<string, HTMLElement>>({});
		const virtualListRef = shallowRef();
		...
		const getFallBackOption = (value: string | number | boolean | Record<string, unknown>): SelectOptionData => {
		...
		const getValueFromValueKeys = (valueKeys: string[]) => {
			if (!props.multiple) {
				return optionInfoMap.get(valueKeys[0])?.value ?? (hasEmptyStringKey(optionInfoMap) ? (undefined as unknown as string) : '');
			}
			...
			for (const item of computedValueObjects.value) {
				...
				if (optionInfo) {
					result.push({
						...optionInfo,
						value: item.key,
						label: optionInfo?.label ?? String(isObject(item.value) ? item.value[valueKey?.value] : item.value),
			...
			return (
				<SelectDropdown
					...
					v-slots={{
						...
						'virtual-list': () => (
							<VirtualList
								{...(props.virtualListProps as any)}
								ref={virtualListRef}
								data={validOptions.value}
								v-slots={{
									item: ({ item }: { item: SelectOptionInfo | SelectOptionGroupInfo }) => renderOption(item),
								}}
							/>
						),
						empty: slots.empty,
		...
		const dropdownVisible = computed(() => {
			if (props.allowSearch && props.hideEmptyInputValue && !computedInputValue.value && validOptions.value.length === 0) {
				return false;
			}
			return computedPopupVisible.value;
		});
