...
hooks.add('tableMenuModule', {
	setupTable($xeTable) {
		...
		const handleOpenMenuEvent = (evnt: any, type: 'header' | 'body' | 'footer', params: any) => {
			...
			const disabledMethod = menuOpts.disabledMethod;
			if (config) {
		...
		menuPrivateMethods = {
			...
			moveCtxMenu(evnt, ctxMenuStore, property, hasOper, operRest, menuList) {
				...
				if (hasOper) {
					...
					(globalEvents.hasKey(evnt, GLOBAL_EVENT_KEYS.ENTER) || globalEvents.hasKey(evnt, GLOBAL_EVENT_KEYS.SPACEBAR))
				) {
			...
			ctxMenuLinkEvent(evnt, menu) {
				...
				if (
					(isFunction(menu.disabled) ? !menu.disabled(internalData._currMenuParams) : !menu.disabled) &&
					...
					const tmMethod = gMenuOpts ? gMenuOpts.tableMenuMethod : null;
					if (tmMethod) {
