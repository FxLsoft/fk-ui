...
import XEUtils, { isString } from 'xe-utils';
import { getElementScrollRect, getParentScroller, getPopupStyle } from '@erp/common';
...
hooks.add('tableFilterModule', {
	setupTable($xeTable) {
		...
		const { refTableFilter, refElem } = $xeTable.getRefMaps();
		...
		const filterPrivateMethods: TableFilterPrivateMethods = {
			...
			triggerFilterEvent(evnt, column, params) {
				...
				if (filterStore.column === column && filterStore.visible) {
					...
					const { target: targetElem }: { target: HTMLElement } = evnt;
					const { filters, filterMultiple, filterRender } = column;
					const compConf = isEnableConf(filterRender) ? renderer.get(filterRender.name) : null;
					const frMethod = column.filterRecoverMethod || (compConf ? compConf.tableFilterRecoverMethod : null);
					...
					nextTick(() => {
						...
						const toTarget = computeFilterOpts.value.teleportTo;
						if (toTarget) {
							if (isString(toTarget)) {
								container = document.body.querySelector(toTarget);
							} else if (toTarget instanceof HTMLElement) {
								container = toTarget;
							}
						}
						if (!container) {
							container = refElem.value;
						}
						const containerRect = container.getBoundingClientRect();
						const triggerRect = getElementScrollRect(targetElem, containerRect);
						const popupRect = getElementScrollRect(refTableFilter.value.$el as HTMLElement, containerRect);
						const { style } = getPopupStyle('bottom', containerRect, triggerRect, popupRect, {
							offset: 12,
							translate: [0, 0],
							customStyle: null,
						...
						filterStore.maxHeight = 260;
						return;
					});
				}
				$xeTable.dispatchEvent(
					'filter-visible',
					{ column, field: column.field, property: column.field, filterList: $xeTable.getCheckedFilters(), visible: filterStore.visible },
					evnt,
				);
			},
			handleClearFilter(column) {
				if (column) {
					...
					if (filters) {
						const compConf = isEnableConf(filterRender) ? renderer.get(filterRender.name) : null;
						const frMethod = column.filterResetMethod || (compConf ? compConf.tableFilterResetMethod : null);
