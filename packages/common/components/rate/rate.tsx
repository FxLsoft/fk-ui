...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const indexArray = computed<undefined[]>(() => [...new Array(props.grading ? 5 : props.count)]);
		...
		const renderCharacter = (index: number) => {
			...
			const leftStyle =
				customColor.value && props.allowHalf && index + 0.5 === displayIndex.value ? { color: customColor.value[parseDisplayIndex] } : undefined;
			const rightStyle = customColor.value && index + 1 <= displayIndex.value ? { color: customColor.value[parseDisplayIndex] } : undefined;
			...
			return (
				<div class={cls} style={style} {...(!props.allowHalf ? getAriaProps(index) : undefined)} onAnimationend={() => handleAnimationEnd(index)}>
					<div class={`${prefixCls}-character-left`} style={leftStyle} {...leftProps} {...(props.allowHalf ? getAriaProps(index, true) : undefined)}>
						{renderElement(index)}
					</div>
					<div class={`${prefixCls}-character-right`} style={rightStyle} {...rightProps} {...(props.allowHalf ? getAriaProps(index) : undefined)}>
						{renderElement(index)}
					</div>
				</div>
			);
