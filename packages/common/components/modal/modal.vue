<template>
	<ClientOnly>
		<Teleport :to="teleportContainer" :disabled="!renderToBody || !show">
			<div
				v-if="needMount"
				v-show="show"
				:class="`${prefixCls}-container`"
				:style="isFixed ? { zIndex } : { zIndex: zIndex, position: 'absolute' }"
				v-bind="$attrs"
			>
				...
				<div ref="wrapperRef" :class="wrapperCls" @click.self="handleMaskClick" @mousedown.self="handleMaskMouseDown">
					<transition :name="modalAnimationName" appear @after-enter="handleOpen" @after-leave="handleClose">
						<div v-show="computedVisible" ref="modalRef" :class="modalCls" :style="mergedModalStyle">
							<div v-if="!hideTitle && ($slots.title || title || closable)" :class="`${prefixCls}-header`" @mousedown="handleMoveDown">
								<div v-if="$slots.title || title" :class="[`${prefixCls}-title`, `${prefixCls}-title-align-${titleAlign}`]">
							...
							<div v-if="footer" :class="footerCls">
								<slot name="footer">
									<template v-if="simple">
										<Button
											v-if="!hideCancel"
											type="info"
											:class="`${prefixCls}-cancel-btn`"
...
<script lang="tsx">
import { computed, defineComponent, onBeforeUnmount, onMounted, ref, shallowRef, toRefs, watch } from 'vue';
...
import { contains, getElement, off, on } from '../_utils/dom';
import usePopupManager from '../_hooks/use-popup-manager';
import { isFunction, isNumber, isPromise } from '../_utils/is';
import { KEYBOARD_KEY } from '../_utils/keyboard';
import { useTeleportContainer } from '../_hooks/use-teleport-container';
import { useDraggable } from './hooks/use-draggable';
import type { ButtonProps } from '../button';
import type { MessageType } from '../_utils/constant';
...
export default defineComponent({
	...
	props: {
		 ...
		 */
		onBeforeOk: {
			type: Function as PropType<(done: (closed: boolean | any) => void) => any | boolean | Promise<any | boolean>>,
		},
		/**
	...
	setup(props, { emit, slots, attrs }) {
		...
		const { handleMoveDown, resetPosition } = useDraggable({
			wrapperRef,
			modalRef,
		...
		const footerCls = computed(() => [`${prefixCls}-footer`, `${prefixCls}-footer-${props.footerAlign}`]);

		const needMount = computed(() => {
			return !props.unmountOnClose || computedVisible.value || mounted.value;
		});

		const show = computed(() => {
			return computedVisible.value || mounted.value;
		});
