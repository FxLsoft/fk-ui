	...
	>
		<slot name="trigger">
			<SelectView
				...
				v-bind="$attrs"
				@input-value-change="onSearchValueChange"
				@clear="onInnerClear"
				@remove="onItemRemove"
				@blur="onBlur"
			>
...
import { isFunction, isObject, isUndefined } from '../_utils/is';
...
import type { Node, TreeFieldNames, TreeNodeData, TreeNodeKey, TreeProps } from '../tree/interface';
...
export default defineComponent({
	...
	props: {
		...
		/**
		 * @zh 是否允许搜索 retainInputValue 为 true 时，输入框不会清空
		 ...
		 * */
		defaultValue: {
			type: [String, Number, Array, Object] as PropType<string | number | Array<string | number> | LabelValue | LabelValue[]>,
		},
		/**
		 * @zh 绑定值
		 * @en Value
		 ...
		 */
		fallbackOption: {
			type: [Boolean, Function] as PropType<boolean | ((key: number | string) => TreeNodeData | boolean)>,
			default: true,
		},
		/**
		 * @zh 设置可选择的节点，默认全部可选
		 ...
		 */
		selectable: {
			type: [Boolean, String, Function] as PropType<boolean | 'leaf' | ((node: TreeNodeData, info: { isLeaf: boolean; level: number }) => boolean)>,
			default: true,
		},
		/**
		 * @zh 是否开启虚拟滚动条
		...
		/**
		 * @zh 标签内容不换行
		 * @en The tag content does not wrap
		 */
		tagNowrap: {
			type: Boolean,
			default: false,
	...
	emits: {
		...
		/**
		 * @zh 失去焦点触发
		 * @en Triggered when the focus is lost
		 * */
	...
	setup(props, { emit, slots }) {
		...
		const retainInputValue = computed(() => isObject(props.allowSearch) && Boolean((props.allowSearch as { retainInputValue?: boolean }).retainInputValue));
		...
		const { selectedKeys, selectedValue, setLocalSelectedKeys, localSelectedKeys, localSelectedValue } = useSelectedState(
			reactive({
				defaultValue,
				modelValue,
				key2TreeNode,
		...
		const refSelectView = shallowRef();
