<template>
	...
	>
		<icon-check v-if="type === 'circle' && size === 'mini' && status === 'success'" :style="{ fontSize: mergedWidth - 2, color }" />
		<svg v-else :viewBox="`0 0 ${mergedWidth} ${mergedWidth}`" :class="`${prefixCls}-svg`">
			<defs v-if="isLinearGradient">
				<linearGradient :id="linearGradientId" x1="0" y1="1" x2="0" y2="0">
					<stop v-for="key of Object.keys(color)" :key="key" :offset="key" :stop-color="color[key]" />
				</linearGradient>
			</defs>
			<circle
				:class="`${prefixCls}-bg`"
...
import type { SIZES } from '../_utils/constant';
import type { PropType } from 'vue';

let __PROGRESS_SEED = 0;
...
export default defineComponent({
	...
	setup(props) {
		...
		const mergedStrokeWidth = computed(() => props.strokeWidth ?? (props.size === 'mini' ? mergedWidth.value / 2 : DEFAULT_STROKE_WIDTH[props.size]));

		const mergedPathStrokeWidth = computed(
			() => props.pathStrokeWidth ?? (props.size === 'mini' ? mergedStrokeWidth.value : Math.max(2, mergedStrokeWidth.value - 2)),
		);
		...
		const linearGradientId = computed(() => {
			__PROGRESS_SEED += 1;
			return `${prefixCls}-linear-gradient-${__PROGRESS_SEED}`;
		});
