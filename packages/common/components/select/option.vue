<template>
	...
	>
		...
		<Checkbox v-if="selectCtx && selectCtx.multiple" :class="`${prefixCls}-checkbox`" :model-value="isSelected" :disabled="disabled" uninject-group-context>
			<slot>{{ label }}</slot>
		</Checkbox>
		<template v-else>
			<span :class="`${prefixCls}-content`" :title="label">
				<slot>{{ label }}</slot>
			</span>
		</template>
...
<script lang="ts">
import { computed, defineComponent, getCurrentInstance, inject, onBeforeUnmount, onMounted, onUpdated, reactive, ref, toRefs, watch } from 'vue';
...
export default defineComponent({
	...
	setup(props) {
		...
		const key = computed(() => getKeyFromValue(value.value, selectCtx?.valueKey));
