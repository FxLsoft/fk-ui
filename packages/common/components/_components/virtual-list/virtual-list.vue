...
<script lang="tsx">
import { computed, defineComponent, nextTick, onUnmounted, reactive, ref, toRefs, watch } from 'vue';
...
import { getItemAbsoluteTop, getScrollPercentage, getValidScrollTop } from './utils';
import type { ItemSlot, ScrollOptions, VirtualItemKey, VirtualListProps } from './interface';
...
export default defineComponent({
	...
	setup(props, { slots, emit }) {
		const { height, itemKey, data, estimatedItemHeight: propEstimatedItemHeight, isStaticItemHeight, threshold } = toRefs(props);
		...
		function getItemKey(item: any, index: number) {
			...
			if (isString(itemKey.value)) {
				...
				result = (itemKey.value as Function)(item);
			}
			return result ?? index;
		}
		...
		const { itemHeight, minItemHeight, totalHeight, setItemHeight, getItemHeight, getItemHeightOrDefault, getItemHeightOrDefaultByIndex } = useItemHeight(
			reactive({
		...
		const isVirtual = computed(
			() =>
				threshold?.value !== null && (threshold?.value === undefined || itemCount.value >= threshold.value) && totalHeight.value > viewportHeight.value,
		);
		...
		const scrollListPadding = computed(() => {
			if (!isUndefined(viewportRef.value)) {
				...
				const getPadding = (property: keyof CSSStyleDeclaration) => +(window.getComputedStyle(viewport)[property] as string).replace(/\D/g, '');
				return {
		...
		const updateScrollOffset = () => {
			...
			let newStartOffset = getItemAbsoluteTop({
				scrollPtg,
				clientHeight,
				scrollTop: scrollTop - (scrollListPadding.value.top + scrollListPadding.value.bottom) * scrollPtg,
